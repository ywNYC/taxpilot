...
// ═══════════════════════════════════════════════════════════
//  TaxPilot · 互动税务策略引擎 · 内测 v3.2 (v136)
//  详细版本日志见同目录 CHANGELOG.md
// ═══════════════════════════════════════════════════════════
...
// v131: 州级 Paid Family & Medical Leave / Temp Disability / SUI（雇员部分）
// 不是联邦税 · 但从工资里扣 · 影响真实到手 · 2025 rates
// 覆盖 XHS 华人主要居住州：东北走廊 + 西海岸 + 主要 IT hub
const STATE_PFML_2025 = {
  NY: { rate: 0.00388, cap: 89343,  label: 'NY PFL',       note: '工资 × 0.388% · 封顶 $346' },
  NJ: { rate: 0.0006,  cap: 165400, label: 'NJ FLI',       note: '工资 × 0.06% · 封顶 $99' },
  MA: { rate: 0.0018,  cap: 176100, label: 'MA PFML',      note: '工资 × 0.18% · 封顶 $317' },
  WA: { rate: 0.00414, cap: 176100, label: 'WA PFML',      note: '雇员 ~0.414% · 封顶 $729' },
  RI: { rate: 0.011,   cap: 89200,  label: 'RI TCI',       note: '工资 × 1.1% · 封顶 $981' },
  CT: { rate: 0.005,   cap: 168600, label: 'CT PFML',      note: '工资 × 0.5% · 封顶 $843' },
  OR: { rate: 0.006,   cap: 168600, label: 'OR Paid Leave',note: '雇员 × 0.6% · 封顶 $1,011' },
  CO: { rate: 0.0045,  cap: 176100, label: 'CO FAMLI',     note: '工资 × 0.45% · 封顶 $793' },
  MD: { rate: 0.0045,  cap: 176100, label: 'MD FAMLI',     note: '2025 生效 · 0.45% 封顶 $793' },
  // CA SDI 已单独处理（1.2% 无上限）
  // DC/DE/NH/VT: 雇员部分 0（雇主全担或无）
};
...
function computeTax(i) {
  ...
  // v115: MFJ + 配偶也有 W2 时 · 401(k) 额度翻倍（夫妻各 $23,500 = $47,000）
  const k401CapEff = (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0)
    ? K401_LIMIT_2025 * 2
    : K401_LIMIT_2025;
  const k401Pretax = Math.min(i.k401 || 0, k401CapEff);
  ...
  // v131: §125 cafeteria plan pre-tax premiums（健康/牙/眼）· 月存入年化
  //   这类 premium 三重减税：联邦 + 州 + FICA（和 401k 不同 · 401k 只减联邦/州 · 不减 FICA）
  //   所以加入 aboveLine（减 AGI）· 并在 FICA 计算前从 ssW2Base 减去
  const healthPremiumAnnual = (i.healthPremium || 0) * 12;
  const dentalPremiumAnnual = (i.dentalPremium || 0) * 12;
  const visionPremiumAnnual = (i.visionPremium || 0) * 12;
  const premiumSpending = healthPremiumAnnual + dentalPremiumAnnual + visionPremiumAnnual;

  const aboveLine = k401Pretax + hsaContrib + seDed + premiumSpending;
  ...
  // v116: MFJ 双工作州支持
  // · 你的 w2 → sourced to workState (if set · else residence state)
  // · 配偶 spouseW2 → sourced to spouseWorkState (if set · else residence state)
  // · 各自在工作州非居民缴税 · 居住州按比例给 credit
  let workStateTax = 0;
  ...
  let spouseWorkStateDetails = null;

  // helper：计算某个配偶工作州的非居民税 · 返回 { workTax, credit, details, isReciprocal, hasConvenience }
  const computeWorkStateSub = (w2Amount, workState, workStateDays) => {
    if (!workState || workState === i.state || !w2Amount) {
      return { workTax: 0, credit: 0, details: null, isReciprocal: false, hasConvenience: false };
    }
    const reciprocalList = RECIPROCAL_AGREEMENTS[i.state] || [];
    if (reciprocalList.includes(workState)) {
      return { workTax: 0, credit: 0, details: null, isReciprocal: true, hasConvenience: false };
    }
    const workRules = STATE_BRACKETS[workState];
    if (!workRules) {
      return { workTax: 0, credit: 0, details: null, isReciprocal: false, hasConvenience: false };
    }
    const rawDays = workStateDays ?? 100;
    const userPortion = Math.max(0, Math.min(100, rawDays)) / 100;
    const hasConv = CONVENIENCE_RULE_STATES[workState] != null;
    const effPortion = hasConv ? 1 : userPortion;
    const workSourceW2 = w2Amount * effPortion;
    // 非居民 std ded 按比例（workPortion × filing std ded）
    const workStdDed = (workRules.stdDed[i.filingStatus] || 0) * effPortion;
    const workTaxable = Math.max(0, workSourceW2 - workStdDed);
    const workBrackets = workRules[i.filingStatus] || workRules.Single;
    const workTax = progressiveTax(workTaxable, workBrackets);

    // 居住州给的 credit · 不超过居住州在这部分 W2 上的税
    let credit = 0;
    if (workSourceW2 > 0 && stateTaxable > 0) {
      ...
      credit = Math.min(workTax, residentTaxOnWorkPortion);
    }
    return {
      workTax,
      credit,
      isReciprocal: false,
      hasConvenience: hasConv,
      details: {
        state: workState,
        name: workRules.label || workRules.name || workState,
        rawTax: workTax,
        credit,
        hasConvenience: hasConv,
        daysPortion: userPortion,
        effectivePortion: effPortion,
  ...
  if (hasWorkState) {
    const mine = computeWorkStateSub(totalW2 - (i.spouseW2 || 0), i.workState, i.workStateDays);
    // 注意 totalW2 包括了 spouse · 这里应该只算你自己 W2 部分 sourced
    // 实际 totalW2 = (i.w2 || 0) + (i.spouseW2 || 0)
    const myW2Only = i.w2 || 0;
    const mineAgain = computeWorkStateSub(myW2Only, i.workState, i.workStateDays);
    workStateTax += mineAgain.workTax;
    crossStateCredit += mineAgain.credit;
    isReciprocal = mineAgain.isReciprocal;
    hasConvenience = mineAgain.hasConvenience;
    effectiveWorkPortion = mineAgain.details?.effectivePortion || 1;
    workStateDetails = mineAgain.details;
  }

  // v116: 配偶工作州（MFJ 且配偶有 W2 且配偶工作州 ≠ 居住州）
  if (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0 && i.spouseWorkState && i.spouseWorkState !== i.state) {
    const spouse = computeWorkStateSub(i.spouseW2 || 0, i.spouseWorkState, i.spouseWorkStateDays);
    workStateTax += spouse.workTax;
    crossStateCredit += spouse.credit;
    if (!isReciprocal) isReciprocal = spouse.isReciprocal;
    if (!hasConvenience) hasConvenience = spouse.hasConvenience;
    spouseWorkStateDetails = spouse.details;
  }
  // 老字段 hasWorkState 含义保留（影响其它 UI） · 扩展条件
  const hasAnyWorkState = hasWorkState ||
    (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0 && i.spouseWorkState && i.spouseWorkState !== i.state);
  ...
  // v131: §125 premiums reduce Box 3/5（SS/Medicare wages） · FICA base 减 premiumSpending
  //   注意：401k 不减 FICA（still subject to FICA）· HSA/DCFSA/commuter 严格讲也减 FICA 但老代码简化未减 · 本版只精确处理 premium
  const ficaW2Base = Math.max(0, totalW2 - premiumSpending);
  const ssTax = Math.min(ficaW2Base, SS_WAGE_BASE_2025) * 0.062;
  const medicareTax = ficaW2Base * 0.0145;
  const addlMedicare = Math.max(0,
    ficaW2Base - (i.filingStatus === 'MFJ' ? 250000 : 200000)
  ) * 0.009;
  const fica = ssTax + medicareTax + addlMedicare;

  // v131: 州级 Paid Leave / TDI · 居住州规则（跨州暂按居住州简化 · 严格应按工作州）
  const pfmlRule = STATE_PFML_2025[i.state];
  const statePFML = pfmlRule ? Math.min(totalW2, pfmlRule.cap) * pfmlRule.rate : 0;

  const totalTax = fedTax + stateTax + localTax + fica + seTax + statePFML;
  const effectiveRate = grossWages > 0 ? totalTax / grossWages : 0;
  const takeHome = grossWages - totalTax;

  // 当下到手现金（真正能花的钱）= Gross - 税 - pre-tax 供款 - 保险 premium
  // v131: §125 premiums 也是工资单上扣的 · 不是 "资产" · 是"花出去的钱"· 从 cashTakeHome 扣
  ...
  const cashTakeHome = takeHome - deferredAssets - premiumSpending;

  // v131: 月/双周到手 · 按 cashTakeHome 除以期数 · 给用户"每月到底多少钱"的直观
  const monthlyTakeHome = cashTakeHome / 12;
  const biweeklyTakeHome = cashTakeHome / 26;
  ...
  return {
    ...
    // v131: 真实工资扣款 · §125 premiums + 州级 PFML
    healthPremiumAnnual, dentalPremiumAnnual, visionPremiumAnnual, premiumSpending,
    statePFML, pfmlRule,
    monthlyTakeHome, biweeklyTakeHome,
    residentStateTax, workStateTax, crossStateCredit, isReciprocal, workStateDetails,
    spouseWorkStateDetails,  // v116 MFJ 双工作州
...
function computeTaxCA(i) {
  ...
  // v131: 月/双周到手 · CA 也一样 · 便于 Hero 显示
  const monthlyTakeHome = cashTakeHome / 12;
  const biweeklyTakeHome = cashTakeHome / 26;
  // CA 无 §125 / PFML 概念 · 置 0 兼容 UI
  const premiumSpending = 0;
  const statePFML = 0;
  const pfmlRule = null;
  ...
  return {
    ...
    // v131: 月/双周到手 · US/CA 统一
    monthlyTakeHome, biweeklyTakeHome, premiumSpending, statePFML, pfmlRule,
    // 加拿大跨州（用不上 · 占位）
...
function findOpportunitiesCA(i, calc) {
  ...
  // ═══ v115 · HST / GST / ITC 机会卡（自雇专属）═══
  // HST/GST 是跟所得税并行的消费税系统 · 过 $30K 要注册 · 3 张关键提示卡
  const inc1099Raw = i.inc1099 || 0;
  const expense1099Raw = i.expense1099 || 0;

  // HST 税率映射（省份）
  const HST_RATES = {
    ON: 0.13, NB: 0.15, NL: 0.15, NS: 0.15, PE: 0.15,      // HST（联邦+省合并）
    BC: 0.05, AB: 0.05, SK: 0.05, MB: 0.05,                // GST-only（PST 分开 · 自雇服务通常 GST-only）
    QC: 0.05,                                               // GST + QST 9.975% · 自雇这里按 GST 算 · QST 另算
    NT: 0.05, YT: 0.05, NU: 0.05,
  };
  // Quick Method 服务业税率（CRA 官定 · 跨省客户 < 90% 的标准）
  const QUICK_METHOD_RATES = {
    ON: 0.088,   // ON 13% HST → 8.8% Quick
    NB: 0.105, NL: 0.105, NS: 0.105, PE: 0.105,  // 15% HST → 10.5% Quick
    BC: 0.036, AB: 0.036, SK: 0.036, MB: 0.036,   // GST 5% → 3.6% Quick
    QC: 0.036, NT: 0.036, YT: 0.036, NU: 0.036,
  };
  const hstRate = HST_RATES[i.state] || 0.13;
  const quickRate = QUICK_METHOD_RATES[i.state] || 0.088;
  const hstRatePct = Math.round(hstRate * 100);

  // 卡 1 · 注册 HST/GST（触发：inc1099 > $30K）
  if (inc1099Raw > 30000) {
    const grossHST = Math.round(inc1099Raw * hstRate);
    const itcHST = Math.round(expense1099Raw * hstRate);
    const netHST = grossHST - itcHST;
    opps.push({
      id: 'hstRegister',
      type: 'warning',
      priority: 1,
      title: '注册 HST/GST · 你过门槛了',
      hook: `自雇 $${fmt(inc1099Raw)} · 过 $30K Small Supplier 门槛`,
      detail: `CRA 规定：连续 4 个滚动季度自雇收入 > $30K 必须注册 HST/GST 号。注册后开票加 ${hstRatePct}% 税、季度/年度申报、净交 CRA（收的 HST 减 ITC 抵扣）。漏注册追 3 年补 + 罚金 + 利息。按你情况估算：收的 HST $${fmt(grossHST)} - ITC 抵 $${fmt(itcHST)} = 净交 $${fmt(netHST)}。CRA My Business Account 10 分钟开户。`,
      contrib: 0,
      saving: 0,
      hold: true,
      code: 'HST',
  ...
  // 卡 2 · Quick Method 估算（触发：自雇 $30K-$400K · 服务业 · 费用比 < 35%）
  if (inc1099Raw > 30000 && inc1099Raw < 400000) {
    const expenseRatio = inc1099Raw > 0 ? expense1099Raw / inc1099Raw : 0;
    const regularNetHST = Math.round((inc1099Raw - expense1099Raw) * hstRate);
    const quickNetHST = Math.round(inc1099Raw * quickRate);
    const diff = regularNetHST - quickNetHST;
    if (expenseRatio < 0.35 && diff > 500) {
      // Quick Method 对低费用率的服务业最划算
      opps.push({
        id: 'hstQuickMethod',
        type: 'info',
        priority: 2,
        title: 'Quick Method · 可能多省 $' + fmt(diff),
        hook: `${i.state} 服务业小户简化选项`,
        detail: `常规法：收 HST $${fmt(Math.round(inc1099Raw * hstRate))} - ITC $${fmt(Math.round(expense1099Raw * hstRate))} = 净交 $${fmt(regularNetHST)}。Quick Method：按营收 × ${(quickRate * 100).toFixed(1)}% 固定比例交 = $${fmt(quickNetHST)}。你费用比 ${(expenseRatio * 100).toFixed(0)}% (低) · Quick 不追 ITC 但税率低 · 估省 $${fmt(diff)}/年。注意：要主动 election（GST74 表格）· 1/1 生效。适合咨询 / 写作 / 设计 / 网红类低费用服务业。`,
        contrib: 0,
        saving: diff,
        hold: true,
        code: 'QuickMethod',
  ...
  // 卡 3 · ITC 可能漏了（触发：自雇 > $30K · 费用比 < 15%）
  if (inc1099Raw > 30000 && expense1099Raw < inc1099Raw * 0.15) {
    const avgExpense = Math.round(inc1099Raw * 0.22);         // 华人自雇平均费用比 18-25%
    const missingExpense = avgExpense - expense1099Raw;
    const missingITC = Math.round(missingExpense * hstRate);
    const missingIncomeTax = Math.round(missingExpense * mr);
    opps.push({
      id: 'itcMissing',
      type: 'warning',
      priority: 2,
      title: 'ITC 可能漏了',
      hook: `费用 $${fmt(expense1099Raw)} vs 自雇 $${fmt(inc1099Raw)}（${Math.round(expense1099Raw / inc1099Raw * 100)}%）`,
      detail: `华人自雇平均费用比 18-25% · 你只记了 ${Math.round(expense1099Raw / inc1099Raw * 100)}% · 可能漏了：手机 / 宽带（业务部分）· Home Office 面积比（通常 10-20%）· 设备 / 软件订阅 · 商务打车 / 加油 · 专业服务费。若补到平均水平（约 $${fmt(avgExpense)}）· 每漏记 $1K 少抵 ${hstRatePct}% = $${hstRate * 1000 | 0} ITC + 多交所得税 $${Math.round(mr * 1000)}。注意：餐饮 / 招待只能抵 50% · 不是 100%。`,
      contrib: 0,
      saving: missingITC + missingIncomeTax,
      hold: true,
      code: 'ITC',
    });
  }
...
function findOpportunities(i, calc) {
 ...
 '报税时在 Schedule C 或 1120-S 申报',
  ...
  // v115: MFJ 且配偶也有 W2 · 401(k) 额度翻倍（夫妻各 $23,500 = $47,000）
  const k401EffCap = (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0)
    ? K401_LIMIT_2025 * 2
    : K401_LIMIT_2025;
  const k401Gap = k401EffCap - (i.k401 || 0);
  ...
  if (totalEarned >= 30000 && k401Gap > 3000) {
    const isMFJDual = i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0;
    opps.push({
      id: 'max401k', title: isMFJDual ? '存满 W2 401(k) · 夫妻各一份' : '存满 W2 401(k)', tag: '退休账户',
      saving: Math.round(k401Gap * mr), contrib: k401Gap,
      urgency: '12/31 前工资扣', difficulty: 1,
      why: isMFJDual
        ? `2025 夫妻各 $${K401_LIMIT_2025.toLocaleString()} · 合计上限 $${(K401_LIMIT_2025 * 2).toLocaleString()}，你还能再存 $${k401Gap.toLocaleString()}`
        : `2025 上限 $${K401_LIMIT_2025.toLocaleString()}，你还能再存 $${k401Gap.toLocaleString()}`,
      how: [
 '联系 HR 调整 pre-tax 401k 百分比',
 isMFJDual ? '夫妻各自在各自雇主 plan 里独立 max · 不共用额度' : '如果工资跟不上，年底几个月可以临时冲一波',
 '如果雇主有 Roth 401k，可分配',
      ],
 ...
 'SSTB (律师/医生/咨询) 有收入上限',
  ...
  //   v130: 改为 opt-in · 默认 p.costSeg 未设 · 仅作为 info 卡显示潜在空间 · 不自动计入省税
  //         用户在 rental 编辑处勾选「已做/计划做 Cost Seg 研究」后 · 才把省税纳入 opp
  //   理由：研究费 $4.5K+ · 需要 engineer 出报告 · recapture 风险 · 不是「自动生效」的机会
  if (rentalProps.length > 0 && totalRentalValue > 300000) {
    ...
    const hasOptIn = rentalProps.some(p => p.costSeg === true);
    if (netBenefit > 5000) {
      if (hasOptIn) {
        // 用户已勾选 · 作为实际可计入的 opp · 进入省税调节器
        opps.push({
        ...
        // 未勾选 · 仅作为信息卡 · 不进调节器 · 不影响 Hero 数字
        opps.push({
          id: 'costSeg', title: '考虑 Cost Seg? · 潜省 $' + fmt(netBenefit), tag: '可选',
          saving: 0, contrib: 0, type: 'info',
          urgency: '先评估 · 报税时（可追溯）', difficulty: 4,
          why: `你的出租房规模够大（总值约 $${fmt(totalRentalValue)}）· 做 Cost Seg 可把第一年折旧多提 ~$${fmt(accelDep)} · 净省税 ~$${fmt(netBenefit)}（已扣研究费 $${fmt(studyCost)}）· 但需前置投入 + recapture 风险 · 所以默认不替你做决定`,
          how: [
            `① 先咨询专业 Cost Seg 公司（免费 feasibility study）`,
            `② 确定净收益 > 研究费后再下单（费用约 $${fmt(studyCost)}）`,
            `③ 报告出来后 · 在房产编辑里勾「已做/计划做 Cost Seg」· 系统才把省税计入`,
            `※ 以前年份也可追溯（Form 3115）· 无需修改往年 return`,
          ],
          warn: '卖房时 §1250 按 25% recapture · 长期持有或 1031 可化解',
        });
      }
    }
  }
...
// v121: 百分比显示 · 整数省小数点 · 非整数保留 1 位 · 例 63 → "63%" · 63.7 → "63.7%"
const fmtPct = (n) => (Math.abs(n - Math.round(n)) < 0.05 ? Math.round(n) : n.toFixed(1)) + '%';
const pct = (n) => `${(n * 100).toFixed(1)}%`;

// v122: 省税调节器优先级表 · 低值 = 高优先级 · 实用派排序
//   HSA 第一（三重税优）· 401(k) 第二 · Solo/Mega 其后 · Roth IRA/Backdoor 更后 · Commuter/DCFSA 最后
//   CA: RRSP 第一 · TFSA 第二 · FHSA/RESP/donation 其后
//   未列出的 id 默认为 99（最低优先级）
const OPP_PRIORITY = {
  // US
  hsa: 10,
  max401k: 20,
...
const oppPriority = (o) => OPP_PRIORITY[o.id] ?? 99;

// v123: 3 种分配模式 · 用户自己选 · 不由系统猜
//   · proportional:「平均分」每个 opp 同时按比例填 · 老 v120-121 行为
//   · priority:「实用派」按 OPP_PRIORITY 顺序填满 · HSA→401(k)→Mega · 默认
//   · efficient:「省税最优」按 saving/contrib 比率排序 · 每块钱省最多税的先填
const ALLOC_MODES = {
  proportional: { label: '平均', desc: '全部同时按比例' },
  priority:     { label: '实用', desc: 'HSA → 401(k) → Mega' },
  efficient:    { label: '最优', desc: '每块钱省最多税先' },
};
const ALLOC_MODE_KEYS = ['proportional', 'priority', 'efficient'];

// v122-123: 按模式 + scale 分配供款
//   · scale > 0: 视模式决定顺序
//     · proportional: 无排序 · 每个 opp 独立 scale × cap
//     · priority / efficient: 按对应 sorter 排序 · 依次填满 · 预算 = maxContribTotal × scale
//   · scale < 0: 所有模式统一按比例撤回（聚合数字精确 · per-opp 估算）
//   · scale = 0: 返回全零
//   返回 { allocs: [{ opp, contrib, saving, fillFrac }], totalContrib, totalSaving }
//   每个 alloc.fillFrac ∈ [-∞, 1] · 用于 UI 显示「已满 / 填 30% / 未开始」
function allocateByMode(contribOpps, scale, mode = 'priority') {
  // 选 sorter · 未知模式默认不排序
  let sorted;
  if (mode === 'priority') {
    sorted = [...contribOpps].sort((a, b) => oppPriority(a) - oppPriority(b));
  } else if (mode === 'efficient') {
    sorted = [...contribOpps].sort((a, b) => {
      const ea = (a.contrib || 0) > 0 ? (a.saving || 0) / a.contrib : 0;
      const eb = (b.contrib || 0) > 0 ? (b.saving || 0) / b.contrib : 0;
      return eb - ea; // DESC · 高效率先
    });
  } else {
    // proportional · 保持原 order · 但我们也按 priority 排序来显示 · 让编号稳定
    sorted = [...contribOpps].sort((a, b) => oppPriority(a) - oppPriority(b));
  }

  if (Math.abs(scale) < 1e-9) {
    const allocs = sorted.map(o => ({ opp: o, contrib: 0, saving: 0, fillFrac: 0 }));
    return { allocs, totalContrib: 0, totalSaving: 0 };
  }

  // 正向 scale
  if (scale > 0) {
    if (mode === 'proportional') {
      // 每个 opp 独立填 scale × cap · 无顺序概念
      const allocs = sorted.map(o => {
        const cap = o.contrib || 0;
        const alloc = cap * scale;
        return {
          opp: o,
          contrib: Math.round(alloc),
          saving: Math.round((o.saving || 0) * scale),
          fillFrac: scale,
        };
      });
      const totalContrib = allocs.reduce((s, a) => s + a.contrib, 0);
      const totalSaving = allocs.reduce((s, a) => s + a.saving, 0);
      return { allocs, totalContrib, totalSaving };
    }
    // priority / efficient · 按排序顺序依次填满 · 预算 = maxContribTotal × scale
    const maxTotal = sorted.reduce((s, o) => s + (o.contrib || 0), 0);
    let remaining = maxTotal * scale;
    const allocs = sorted.map(o => {
      const cap = o.contrib || 0;
      const alloc = Math.max(0, Math.min(remaining, cap));
      remaining = Math.max(0, remaining - alloc);
      const fillFrac = cap > 0 ? alloc / cap : 0;
      return {
        opp: o,
        contrib: Math.round(alloc),
        saving: Math.round((o.saving || 0) * fillFrac),
        fillFrac,
      };
    });
    const totalContrib = allocs.reduce((s, a) => s + a.contrib, 0);
  ...
  // 倒推 · scale < 0 · 所有模式统一按比例撤回（聚合精确 · per-opp 估算）
  const allocs = sorted.map(o => ({
    opp: o,
    contrib: Math.round((o.contrib || 0) * scale),
    saving: Math.round((o.saving || 0) * scale),
    fillFrac: scale,
  }));
  const totalContrib = allocs.reduce((s, a) => s + a.contrib, 0);
  ...
  <div style={{ marginBottom: 7 }}>
    <div style={{
      ...
      <label style={{
        ...
        // v126: 允许换行 · 去掉 ellipsis 截断 · 长 label 自动折行
...
// v132: Slider 带「再存/撤回」delta 提示
//   用于 Wizard Step 4 / InputPanel deduct 的 k401/hsa slider
//   解决歧义：「这个值是已存总量 · 还是今年再存多少？」
//   实际语义：这个值 = 全年总供款 · 拖动 = 调整全年总额
//   当 value 与 baseline（用户初次看到时的值）有差异时 · 显示「已存 $X · 再存 +$Y」
//   baseline 可由 parent 显式传入（例如 YTD 估算应用后 reset）· 否则用 useRef 锁死初始值
const ContribEditSlider = ({ label, value, onChange, baseline, ...rest }) => {
  const initialRef = React.useRef(value);
  const effectiveBaseline = baseline != null ? baseline : initialRef.current;
  const delta = (value || 0) - (effectiveBaseline || 0);

  return (
    <div>
      <Slider label={label} value={value} onChange={onChange} {...rest} />
      {Math.abs(delta) >= 1 && (
        <div style={{
          fontSize: 9, color: C.mute, fontFamily: F_BODY,
          marginLeft: 100, marginTop: -2, marginBottom: 4,
          lineHeight: 1.3,
        }}>
          {effectiveBaseline > 0 && (
            <>
              已存 <b style={{ color: C.ink2, fontFamily: F_NUM }}>${fmt(effectiveBaseline)}</b>
              <span style={{ color: C.muteLite }}> · </span>
            </>
          )}
          <span style={{ color: delta > 0 ? C.save : C.pay, fontWeight: 700 }}>
            {delta > 0
              ? `再存 +$${fmt(delta)}`
              : `减 −$${fmt(Math.abs(delta))}`
            }
          </span>
          <span style={{ color: C.muteLite, marginLeft: 6 }}>
            · 年度总 <b style={{ color: C.ink2, fontFamily: F_NUM }}>${fmt(value)}</b>
          </span>
        </div>
      )}
    </div>
...
// v128: YTD 供款估算器 · 解决用户「不知精确已存 $$，但知每张工资存 6% 或 $200」的情况
// 放在 k401 / hsa Slider 下方 · 折叠式展开 · 填数→应用
// 计算：periodsElapsed = floor(totalPeriods × daysElapsed / 365) · YTD = perPeriod × periodsElapsed
const ContribYTDCalc = ({ label, annualBase, maxCap, onApply, defaultMode = 'pct', defaultPct = 6, defaultAmt = 200, defaultFreq = 'biweekly' }) => {
  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState(defaultMode);
  const [pctValue, setPctValue] = useState(defaultPct);
  const [amtValue, setAmtValue] = useState(defaultAmt);
  ...
  const FREQ_LABELS = {
    weekly: '每周 · 52 期',
    biweekly: '双周 · 26 期',
    semimonthly: '月 2 次 · 24 期',
    monthly: '每月 · 12 期',
  };

  const totalPeriods = PAY_PERIODS[freq];
  const perPeriod = mode === 'pct'
    ? (annualBase * pctValue / 100) / totalPeriods
    : amtValue;

  // 已过薪期数 · 按日期线性近似（不考虑具体 paydate 日历）
  const selectedDate = new Date(dateStr);
  const year = selectedDate.getFullYear();
  const yearStart = new Date(year, 0, 1);
  const daysElapsed = Math.max(0, Math.floor((selectedDate.getTime() - yearStart.getTime()) / 86400000));
  const daysInYear = ((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0) ? 366 : 365;
  const periodsElapsed = Math.floor(totalPeriods * daysElapsed / daysInYear);
  const estimatedYTD = Math.round(perPeriod * periodsElapsed);
  const cappedYTD = Math.min(estimatedYTD, maxCap || estimatedYTD);
  ...
  return (
    <div style={{ marginTop: -2, marginBottom: 10 }}>
      ...
      >
        ...
        <span>{open ? '收起估算' : '不知道存了多少？按工资估算'}</span>
      </button>
      {open && (
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              <button key={k} onClick={() => setMode(k)} style={{
                ...
                background: mode === k ? C.info : 'transparent',
                color: mode === k ? '#FFF' : C.mute,
                border: 'none',
                fontSize: 9, fontFamily: F_BODY, fontWeight: 600,
                cursor: 'pointer',
              }}>
                {l}
              </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 6 }}>
            <span style={{ color: C.mute, fontSize: 9, minWidth: 38 }}>每期存</span>
            {mode === 'pct' ? (
              <>
                <input
                  type="number" step="0.5" min="0" max="50"
                  defaultValue={pctValue}
                  onBlur={e => setPctValue(Math.max(0, Math.min(50, Number(e.target.value) || 0)))}
                  style={{
                    width: 48, padding: '2px 5px',
                    border: `1px solid ${C.line}`, borderRadius: 3,
                    fontFamily: F_NUM, fontSize: 11, textAlign: 'right',
                  }}
                />
                <span style={{ color: C.mute, fontSize: 9 }}>% · 薪 ${fmt(annualBase)}</span>
              </>
            ) : (
              <>
                <span style={{ color: C.mute, fontSize: 9 }}>$</span>
                <input
                  type="number" step="10" min="0"
                  defaultValue={amtValue}
                  onBlur={e => setAmtValue(Math.max(0, Number(e.target.value) || 0))}
                  style={{
                    width: 64, padding: '2px 5px',
                    border: `1px solid ${C.line}`, borderRadius: 3,
                    fontFamily: F_NUM, fontSize: 11, textAlign: 'right',
                  }}
                />
          ...
          {/* 频率 */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 6 }}>
            <span style={{ color: C.mute, fontSize: 9, minWidth: 38 }}>发薪</span>
            <select
              value={freq}
              ...
              style={{
                ...
                border: `1px solid ${C.line}`, borderRadius: 3,
                fontFamily: F_BODY, fontSize: 10, background: C.card,
                color: C.ink2,
              }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 8 }}>
            <span style={{ color: C.mute, fontSize: 9, minWidth: 38 }}>截至</span>
            <input
              type="date"
              value={dateStr}
          ...
          <div style={{
            ...
            gap: 8, paddingTop: 6, borderTop: `1px dashed ${C.line}`,
          }}>
            <div style={{ fontSize: 9, color: C.mute, lineHeight: 1.3 }}>
              已过 <b style={{ color: C.ink2, fontFamily: F_NUM }}>{periodsElapsed}/{totalPeriods}</b> 期
              <br />
              估 YTD
              <span style={{
                fontFamily: F_NUM, fontWeight: 700, fontSize: 13,
                color: C.info, marginLeft: 5,
              }}>
                ${fmt(cappedYTD)}
              </span>
              {estimatedYTD > (maxCap || Infinity) && (
                <span style={{ color: C.warn, fontSize: 8, marginLeft: 4 }}>· 已到上限</span>
              )}
            </div>
            <button
              onClick={() => { onApply(cappedYTD); setOpen(false); }}
              style={{
                padding: '5px 12px',
...
// v131: 工资单真实扣款段 · §125 保险 premium + 州级 PFML 信息
//   用户填月度金额 · 内部年化 · 自动减 federal/state/FICA 三重
//   默认折叠 · 用户主动展开填 · 不填默认 0（对结果无影响）
const PayrollBenefitsSection = ({ draft, update }) => {
  const [open, setOpen] = useState(false);
  const pfmlRule = STATE_PFML_2025[draft.state];
  const totalW2 = (draft.w2 || 0) + (draft.spouseW2 || 0);
  const pfmlEst = pfmlRule ? Math.min(totalW2, pfmlRule.cap) * pfmlRule.rate : 0;
  const premiumsMonthly = (draft.healthPremium || 0) + (draft.dentalPremium || 0) + (draft.visionPremium || 0);
    ...
    <div style={{ marginTop: 6, marginBottom: 10 }}>
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span>工资单扣款 · 保险 / 州 PFML</span>
        </span>
        <span style={{ fontSize: 9, color: C.mute, fontFamily: F_MONO }}>
          {premiumsMonthly > 0 ? `$${fmt(premiumsMonthly)}/月` : '不填也行'}
        </span>
      </button>
      ...
      {open && (
        <div style={{
          ...
          {pfmlRule ? (
            <div style={{
              ...
              <div style={{ marginBottom: 2 }}>
                <span style={{ color: C.ink2, fontWeight: 600 }}>{pfmlRule.label}</span>
                <span> · 自动估算 </span>
                <b style={{ fontFamily: F_NUM, color: C.ink }}>${fmt(Math.round(pfmlEst))}</b>
                <span style={{ color: C.muteLite }}> / 年</span>
              </div>
              <div style={{ color: C.muteLite, fontSize: 9 }}>§ {pfmlRule.note} · 从工资自动扣 · 已计入总税负</div>
            </div>
          ) : (
            <div style={{
              padding: '6px 8px', marginBottom: 10,
              fontSize: 10, color: C.muteLite, fontFamily: F_BODY, lineHeight: 1.4,
            }}>
              § 你的居住州无雇员端 PFML 扣款（或雇主全担）
            </div>
          )}
          ...
          <div style={{
            ...
            Section 125 保险 · 预税扣（减联邦+州+FICA）
          </div>
          <Slider
            label="医保月费 PPO"
            value={draft.healthPremium || 0}
            onChange={(v) => update({ healthPremium: v })}
            min={0} max={2000} step={25}
            hint="你出的那部分 · 不含雇主承担 · 月度"
          />
          <Slider
            label="牙科月费"
            value={draft.dentalPremium || 0}
            onChange={(v) => update({ dentalPremium: v })}
            min={0} max={200} step={5}
            hint="可选 · 月度"
          />
          <Slider
            label="视力月费"
            value={draft.visionPremium || 0}
            onChange={(v) => update({ visionPremium: v })}
          ...
          {premiumsMonthly > 0 && (
            <div style={{
              ...
              <span>合计月扣</span>
              <span style={{ fontFamily: F_NUM, fontWeight: 700 }}>
                ${fmt(premiumsMonthly)}/月 · <span style={{ color: C.mute }}>${fmt(premiumsMonthly * 12)}/年</span>
              </span>
            </div>
          )}
        </div>
...
const PropertyCard = ({ property, onChange, onDelete, canDelete, index, country, residenceState }) => {
  ...
  return (
    ...
    >
      ...
      {/* 所在州（无 label 单行） · v133 加 country + stale state 兜底 */}
      <div style={{ marginBottom: 8 }}>
        <StateSelect
          value={(() => {
            if (country === 'CA' && property.state && !CA_PROV_BRACKETS[property.state]) return residenceState;
            if (country !== 'CA' && property.state && CA_PROV_BRACKETS[property.state]) return residenceState;
            return property.state || residenceState;
          })()}
      ...
      {property.type === 'rental' && (
        <div style={{ borderTop: `1px dashed ${C.lineLite}`, paddingTop: 6, marginTop: 4 }}>
          ...
          <Slider
            ...
            min={0} max={60000} step={500}
          />
          {/* v130: Cost Seg opt-in toggle · 默认 false · 勾选后才把 netBenefit 纳入省税 opp */}
          <div style={{ marginTop: 4 }}>
            <Toggle
              label="已做/计划做 Cost Seg"
              value={!!property.costSeg}
              onChange={(v) => update('costSeg', v)}
              hint="加速折旧 · 研究费 ~$4.5K · 勾选后省税才被计入"
            />
          </div>
        </div>
      )}
  ...
  return (
    <div className="rounded-2xl mb-2 overflow-hidden" style={{ background: C.card, border: `1px solid ${C.line}` }}>
      ...
      <div style={{ padding: '10px 12px' }}>
        ...
        {activeTab === 'work' && !isCA && (
          <>
            {/* v124: 工作州 tab 重构 · 3 个选择器同屏显示 · MFJ 跨州清晰化
                     居住州 / 你的工作州 / 配偶工作州 · 后两个默认 = 居住州
                     默认语义:「工作州 === 居住州」= 本州上班 · 选不同 = 触发跨州 */}
            ...
            <div className="mb-3" style={{
              ...
              <StateSelect
                value={i.workState || i.state}
                onChange={(v) => {
                  if (v === i.state) {
                    // 选了居住州 = 本州上班 · 清空 workState/days
                    setI({ ...i, workState: '', workStateDays: undefined });
                  } else {
                    // 选了不同州 = 跨州 · 保留/初始化 days
                    setI({ ...i, workState: v, workStateDays: i.workStateDays ?? 100 });
                  }
                }}
              />

              {/* 状态提示 · 本州 vs 跨州 */}
              {(!i.workState || i.workState === i.state) ? (
                <div style={{
                  fontSize: 10, fontFamily: F_BODY, marginTop: 8,
                  padding: '6px 10px', borderRadius: 6,
                  background: C.saveBg, color: C.save, lineHeight: 1.5,
                }}>
                  ✓ 你本州上班 · 无跨州税务问题
                </div>
              ) : (
            ...
            {/* === 3. 配偶的工作州 · 仅 MFJ + spouseW2 > 0 === */}
            {i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0 && (
              <div className="mb-3" style={{
                padding: '10px 12px',
                border: `1px solid ${C.line}`,
                borderRadius: 8,
                ...
                <StateSelect
                  value={i.spouseWorkState || i.state}
                  onChange={(v) => {
                    if (v === i.state) {
                      setI({ ...i, spouseWorkState: '', spouseWorkStateDays: undefined });
                    } else {
                      setI({ ...i, spouseWorkState: v, spouseWorkStateDays: i.spouseWorkStateDays ?? 100 });
                    }
                  }}
                />

                {/* 状态提示 */}
                {(!i.spouseWorkState || i.spouseWorkState === i.state) ? (
                  <div style={{
                    fontSize: 10, fontFamily: F_BODY, marginTop: 8,
                    padding: '6px 10px', borderRadius: 6,
                    background: C.saveBg, color: C.save, lineHeight: 1.5,
                  ...
                  <>
                    {(() => {
                      const reciprocal = (RECIPROCAL_AGREEMENTS[i.state] || []).includes(i.spouseWorkState);
                      const hasConv = CONVENIENCE_RULE_STATES[i.spouseWorkState] != null;
                      return (
                        <div style={{
                          fontSize: 10, fontFamily: F_BODY, marginTop: 8,
                          padding: '6px 10px', borderRadius: 6,
                          ...
                          {reciprocal
                            ? '✓ Reciprocal 互惠协议：配偶工作州不扣税，只在居住州交'
                            : '† 配偶两州都需报税；居住州会按配偶在工作州缴的税给抵免'}
                        </div>
                      );
                    })()}

                    {!(RECIPROCAL_AGREEMENTS[i.state] || []).includes(i.spouseWorkState) && (
                      <div className="mt-3 pt-3" style={{ borderTop: `1px dashed ${C.lineLite}` }}>
                        <Slider
                          label={`配偶物理在 ${STATE_BRACKETS[i.spouseWorkState]?.label || i.spouseWorkState} 工作天数`}
                          value={i.spouseWorkStateDays ?? 100}
                          onChange={(v) => update('spouseWorkStateDays', v)}
                          min={0} max={100} step={5}
                          format={(n) => `${n}%`}
                          prefix=""
                          hint={CONVENIENCE_RULE_STATES[i.spouseWorkState]
                            ? '† 此州有 Convenience 规则 · 远程天数不算减'
                            : '100 = 全部通勤 · 0 = 全远程'}
                        />
                        {/* 高税居住州 · credit 完全抵免提示 */}
                        {['NY', 'CA', 'NJ', 'OR', 'MN', 'DC'].includes(i.state) && (
                          <div style={{
                            fontSize: 10, fontFamily: F_BODY, marginTop: 6,
                            padding: '6px 10px', borderRadius: 6,
                            background: C.infoBg, color: C.info, lineHeight: 1.5,
                          }}>
                            ¶ {STATE_BRACKETS[i.state]?.label} 是高税州 · 居住州抵免通常能完全抵掉配偶工作州的非居民税 · 总州税可能不变（这是正常的 · 叫"credit for taxes paid to other state"）
                          </div>
                        )}
                      </div>
                    )}
            ...
            <div className="mb-3">
              <div style={{
                fontSize: 10, color: C.mute, fontFamily: F_BODY,
                fontWeight: 600, letterSpacing: '0.1em', marginBottom: 6,
                textTransform: 'uppercase', display: 'flex', alignItems: 'baseline', gap: 6,
              }}>
                <span>快捷场景</span>
                <span style={{ fontSize: 8, fontWeight: 400, color: C.muteLite, textTransform: 'none', letterSpacing: 0 }}>
                  · 只调「你的工作州」· 配偶不变
                </span>
              </div>
              <div className="flex gap-1.5 flex-wrap">
                {[
                  { id: 'onsite', label: '本州上班', desc: '住哪工作哪',
                    apply: () => setI({ ...i, workState: '', workStateDays: undefined }),
                  ...
                  { id: 'hybrid', label: '混合 WFH', desc: '一半远程',
                    apply: () => setI({ ...i, workState: i.workState && i.workState !== i.state ? i.workState : (i.state === 'NY' ? 'NJ' : 'NY'), workStateDays: 50 }),
                    isActive: i.workState && i.workState !== i.state && (i.workStateDays ?? 100) >= 30 && (i.workStateDays ?? 100) < 80 },
                  { id: 'fullWFH', label: '100% WFH', desc: '远程 + 跨州雇主',
                    apply: () => setI({ ...i, workState: i.workState && i.workState !== i.state ? i.workState : (i.state === 'NY' ? 'NJ' : 'NY'), workStateDays: 0 }),
                    isActive: i.workState && i.workState !== i.state && (i.workStateDays ?? 100) < 30 },
                ].map(s => (
                  <button
                    key={s.id}
                    onClick={s.apply}
        ...
        {activeTab === 'deduct' && (
          <>
            ...
            <div style={{ borderTop: `1px dashed ${C.lineLite}`, paddingTop: 10, marginTop: 4 }}>
              ...
              <ContribEditSlider
                label={isCA ? '今年已存 RRSP' : (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0 ? '今年已存 401(k) · 夫妻合计' : '今年已存 401(k)')}
                value={i.k401}
                onChange={(v) => update('k401', v)}
                baseline={i._k401Baseline}
                min={0} max={isCA ? CA_RRSP_LIMIT : (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0 ? K401_LIMIT_2025 * 2 : K401_LIMIT_2025)} step={500}
                hint={isCA
                  ? `2025 上限 $${CA_RRSP_LIMIT.toLocaleString()} 或 18% × 上年收入`
                  : (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0
                      ? `2025 夫妻各 $${K401_LIMIT_2025.toLocaleString()} · 合计上限 $${(K401_LIMIT_2025 * 2).toLocaleString()}`
              ...
              {!isCA && (
                <>
                  ...
                  />
                  {/* v131: §125 保险 premium + 州 PFML · 可选补填 · 让 Hero 月到手更真 */}
                  <PayrollBenefitsSection
                    draft={i}
                    update={(patch) => {
                      Object.entries(patch).forEach(([k, v]) => update(k, v));
                    }}
                  />
                </>
              )}
...
// ═══════════════════════════════════════════════════════════
//  v140: Paystub 模态框 · ADP 风格彩蛋
//  纯娱乐 · 把 calc 数据按周期摊成仿真 paystub · 给懂行的人看看
//  数字按 payFreq 除 · YTD 用「今天距 Jan 1 天数 / 365」比例
// ═══════════════════════════════════════════════════════════

const PaystubModal = ({ calc, payFreq, isCA, onClose }) => {
  const periodsMap = { weekly: 52, biweekly: 26, semimonthly: 24, monthly: 12 };
  const periods = periodsMap[payFreq] || 26;
  const periodLabel = { weekly: 'WEEKLY', biweekly: 'BI-WEEKLY', semimonthly: 'SEMI-MONTHLY', monthly: 'MONTHLY' }[payFreq] || 'BI-WEEKLY';

  // YTD 比例 · 按今天距 Jan 1 的天数
  const today = new Date();
  const jan1 = new Date(today.getFullYear(), 0, 1);
  const daysElapsed = Math.max(1, Math.floor((today - jan1) / (1000 * 60 * 60 * 24)));
  const ytdFrac = Math.min(1, daysElapsed / 365);

  const per = (annual) => (annual || 0) / periods;
  const ytd = (annual) => (annual || 0) * ytdFrac;
  const fmtMoney = (n) => {
    const v = Math.round(n * 100) / 100;
    return v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  };

  // 模拟 pay period 日期（用今天往前推 1 个周期）
  ...
  // Earnings
  const totalW2 = calc.totalW2 || 0;
  const net1099 = calc.net1099 || 0;
  const hourlyRate = totalW2 / 2080; // 标准年工时 52×40
  const hoursPerPeriod = (2080 / periods).toFixed(2);

  // Taxes
  const fedTax = calc.fedTax || 0;
  const ssTax = calc.ssTax || 0;
  const medicareTax = calc.medicareTax || 0;
  const stateTax = calc.stateTax || 0;
  const localTax = calc.localTax || 0;
  const statePFML = calc.statePFML || 0;
  const addlMedicare = calc.addlMedicare || 0;

  // Pre-tax deductions
  const k401 = calc.k401Pretax || 0;
  const hsa = calc.hsaContrib || 0;
  const healthPrem = calc.healthPremiumAnnual || 0;
  const dentalPrem = calc.dentalPremiumAnnual || 0;
  const visionPrem = calc.visionPremiumAnnual || 0;

  // Net per period（严格来说 = gross - taxes - deductions · 不能直接用 cashTakeHome/periods · 因为 cashTakeHome 包含其他 income）
  // 简化：per 周期 net = (totalW2 - 对应税 - 对应扣) / periods
  const periodTaxes = fedTax + ssTax + medicareTax + stateTax + localTax + statePFML + addlMedicare;
  const periodDeducts = k401 + hsa + healthPrem + dentalPrem + visionPrem;
  const periodNet = totalW2 - periodTaxes - periodDeducts;

  // CA 用不同 label
  const ssLabel = isCA ? 'CPP' : 'Social Security';
  const medLabel = isCA ? '' : 'Medicare';

  const earningsLines = [
    totalW2 > 0 && { desc: 'Regular', hours: hoursPerPeriod, rate: hourlyRate.toFixed(4), current: per(totalW2), ytd: ytd(totalW2) },
    net1099 > 0 && { desc: '1099 Contract', hours: '—', rate: '—', current: per(net1099), ytd: ytd(net1099) },
  ].filter(Boolean);

  const taxLines = [
    fedTax > 0 && { desc: 'Fed Income Tax', current: per(fedTax), ytd: ytd(fedTax) },
    ssTax > 0 && { desc: `${ssLabel} (${isCA ? '5.95%' : '6.2%'})`, current: per(ssTax), ytd: ytd(ssTax) },
    medicareTax > 0 && { desc: 'Medicare (1.45%)', current: per(medicareTax), ytd: ytd(medicareTax) },
    addlMedicare > 0 && { desc: 'Addl Medicare (0.9%)', current: per(addlMedicare), ytd: ytd(addlMedicare) },
    stateTax > 0 && { desc: `State ${isCA ? 'Prov.' : 'Tax'}`, current: per(stateTax), ytd: ytd(stateTax) },
    localTax > 0 && { desc: 'Local / City', current: per(localTax), ytd: ytd(localTax) },
    statePFML > 0 && { desc: calc.pfmlRule?.label || 'State PFML', current: per(statePFML), ytd: ytd(statePFML) },
  ].filter(Boolean);

  const deductLines = [
    k401 > 0 && { desc: isCA ? 'RRSP' : '401(k) Pre-Tax', current: per(k401), ytd: ytd(k401) },
    hsa > 0 && { desc: 'HSA', current: per(hsa), ytd: ytd(hsa) },
    healthPrem > 0 && { desc: 'Medical (PPO)', current: per(healthPrem), ytd: ytd(healthPrem) },
    dentalPrem > 0 && { desc: 'Dental', current: per(dentalPrem), ytd: ytd(dentalPrem) },
    visionPrem > 0 && { desc: 'Vision', current: per(visionPrem), ytd: ytd(visionPrem) },
  ].filter(Boolean);

  const row = (l, bold) => (
    <div key={l.desc} style={{
      display: 'grid',
      ...
      borderBottom: bold ? `1px solid ${C.ink}` : `1px dotted ${C.lineLite}`,
    }}>
      <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.desc}</span>
      <span style={{ textAlign: 'right', minWidth: 60 }}>${fmtMoney(l.current)}</span>
      <span style={{ textAlign: 'right', minWidth: 70, color: bold ? C.ink : C.mute }}>${fmtMoney(l.ytd)}</span>
    </div>
  );

  const sectionHeader = (label) => (
    <div style={{
      fontSize: 8, color: C.ink, fontFamily: F_MONO, fontWeight: 700,
      letterSpacing: '0.12em', marginTop: 10, marginBottom: 4,
      paddingBottom: 2, borderBottom: `1px solid ${C.ink}`,
    }}>
      {label}
    </div>
  );
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          paddingBottom: 10, borderBottom: `2px solid ${C.ink}`, marginBottom: 10,
        }}>
          <div>
            <div style={{
              fontSize: 10, fontWeight: 700, letterSpacing: '0.2em',
              color: C.ink, marginBottom: 2,
            }}>PAYROLL STATEMENT</div>
            <div style={{ fontSize: 14, fontFamily: F_NUM, fontWeight: 700, letterSpacing: '-0.01em' }}>
              PaySure<span style={{ color: '#B5351E' }}>®</span> Inc.
            </div>
            <div style={{ fontSize: 8, color: C.mute, marginTop: 1 }}>
          ...
          <button
            ...
            style={{
              background: 'transparent', border: 'none',
              fontSize: 18, color: C.mute, cursor: 'pointer',
        ...
        {/* Employee + period info · 2 列 */}
        <div style={{
          display: 'grid', gridTemplateColumns: '1fr 1fr',
          gap: 10, fontSize: 9, lineHeight: 1.55,
          padding: '8px 0', marginBottom: 6,
          borderBottom: `1px solid ${C.line}`,
        }}>
          <div>
            <div style={{ color: C.mute, letterSpacing: '0.06em' }}>EMPLOYEE</div>
            <div style={{ fontWeight: 700, color: C.ink }}>{isCA ? 'MAPLE LEAF, USER' : 'DOE, JANE Q.'}</div>
            <div style={{ color: C.mute }}>EMP# 00042 · SSN ***-**-1234</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ color: C.mute, letterSpacing: '0.06em' }}>PERIOD</div>
            <div style={{ fontWeight: 700, color: C.ink }}>{fmtDate(periodStart)} – {fmtDate(periodEnd)}</div>
            <div style={{ color: C.mute }}>PAY DATE {fmtDate(payDate)} · {periodLabel}</div>
          </div>
        </div>
        ...
        <div style={{
          ...
          fontSize: 8, color: C.mute, fontWeight: 700,
          letterSpacing: '0.1em',
          borderBottom: `1px solid ${C.mute}`,
        }}>
          <span>DESCRIPTION</span>
          <span style={{ textAlign: 'right', minWidth: 60 }}>CURRENT</span>
          <span style={{ textAlign: 'right', minWidth: 70 }}>YTD</span>
        </div>
        ...
        {earningsLines.length > 0 && (
          <>
            ...
            {earningsLines.map(l => row(l))}
            {row({ desc: 'GROSS PAY', current: per(totalW2 + net1099), ytd: ytd(totalW2 + net1099) }, true)}
          </>
        )}
        ...
        {taxLines.length > 0 && (
          <>
            {sectionHeader('TAXES WITHHELD')}
            {taxLines.map(l => row(l))}
            {row({ desc: 'TOTAL TAXES', current: per(periodTaxes), ytd: ytd(periodTaxes) }, true)}
          </>
        )}
        ...
        {deductLines.length > 0 && (
          <>
            {sectionHeader('PRE-TAX DEDUCTIONS')}
            {deductLines.map(l => row(l))}
            {row({ desc: 'TOTAL DEDUCTIONS', current: per(periodDeducts), ytd: ytd(periodDeducts) }, true)}
          </>
        )}
        ...
        <div style={{
          ...
          background: C.cardAlt, border: `2px solid ${C.ink}`,
          display: 'grid', gridTemplateColumns: '1fr auto auto',
          gap: 10, fontSize: 12, fontFamily: F_MONO,
          alignItems: 'center',
        }}>
          <span style={{ fontWeight: 700, letterSpacing: '0.12em' }}>NET PAY</span>
          <span style={{ textAlign: 'right', fontWeight: 700, fontFamily: F_NUM, fontSize: 15, color: C.save, minWidth: 60 }}>
            ${fmtMoney(periodNet / periods)}
          </span>
          <span style={{ textAlign: 'right', fontWeight: 700, fontFamily: F_NUM, fontSize: 13, color: C.save, minWidth: 70 }}>
            ${fmtMoney(ytd(periodNet))}
          </span>
        </div>
        ...
        <div style={{
          ...
          fontSize: 8, color: '#8B6914', lineHeight: 1.5, fontFamily: F_BODY,
          letterSpacing: '0.02em',
        }}>
          <b>※ 仿真 · 非真实薪资单</b> · 仅供直观展示税务结构
          <br/>
          · 数字基于你当前输入的年度数据按期摊销 · YTD 按今天距 1/1 的比例
          <br/>
          · 实际 paystub 会有具体 hours/rate · 不同雇主代扣策略略异
        </div>
      </div>
    </div>
  );
...
const SummaryCard = ({ calc, i, theme = THEMES.sage, preset }) => {
  ...
  // v132: 月到手详情展开态
  const [takehomeDetailOpen, setTakehomeDetailOpen] = useState(false);
  // v137: 发薪频率 · 4 格联动 · 默认月
  const [payFreq, setPayFreq] = useState('monthly');
  // v140: 仿真 paystub 彩蛋 · 懂的人点开看
  const [paystubOpen, setPaystubOpen] = useState(false);

  // v134: 按来源精确 marginal-rate 拆分月到手
  //   每种收入的「下一美元」税率不同：
  //     · W2: 联邦 + 州 + FICA（SS 6.2% up to cap · Medicare 1.45% 无上限 · Addl 0.9% 高收入）
  //     · 1099: 联邦 + 州 + SE tax (15.3% × 0.9235 up to cap · 2.9% × 0.9235 above · minus half SE deduction)
  //     · 房租净: 联邦 + 州（无 FICA · 无 SE · passive income）
  //     · 投资 (优惠): LTCG rate (0/15/20%) + 州
  //     · 投资 (普通): 联邦 + 州（interest + ordinary div + ST gains）
  //   CA 模式略不同：SS→CPP · Medicare→无 · SE→CPP 自雇 · 统一用 marginalCombined
  //   计算完 impliedNet · 归一到 cashTakeHome 让总和一致
  const grossTotal = calc.grossWages || 1;
  const w2Gross = calc.totalW2 || 0;
  const i1099Gross = calc.net1099 || 0;
  const rentalGross = calc.rentalGainToAGI || 0;
  const prefIncome = calc.preferentialIncome || 0;  // 优惠率部分（qualified div + LT cap gains）
  const ordInvestIncome = calc.ordinaryInvestIncome || 0;  // 普通率部分（interest + ordinary div + ST gains）
  const investGross = prefIncome + ordInvestIncome;
  const monthlyCash = (calc.monthlyTakeHome || 0);
  const biweeklyCash = (calc.biweeklyTakeHome || 0);
  const weeklyCash = monthlyCash * 12 / 52;
  const semimonthlyCash = monthlyCash / 2;

  // v137: 按当前 payFreq 取对应数值 + 单位标签
  const payFreqMap = {
    weekly:      { value: weeklyCash,      unit: '周',   fullLabel: '每周 w',   periods: '52 期' },
    biweekly:    { value: biweeklyCash,    unit: '双周', fullLabel: '双周 bw',  periods: '26 期' },
    semimonthly: { value: semimonthlyCash, unit: '月半', fullLabel: '月半 sm',  periods: '24 期' },
    monthly:     { value: monthlyCash,     unit: '月',   fullLabel: '每月 m',   periods: '12 期' },
  };
  const currentFreq = payFreqMap[payFreq] || payFreqMap.monthly;

  const marginalFed = calc.marginalFed || 0;
  const marginalState = calc.marginalState || 0;
  const filingStatus = i?.filingStatus || 'Single';
  const addlMedicareThreshold = filingStatus === 'MFJ' ? 250000 : (filingStatus === 'MFS' ? 125000 : 200000);

  // W2 marginal · US/CA 分别处理
  const w2MarginalRate = (() => {
    if (isCA) return calc.marginalCombined || (marginalFed + marginalState);  // 包含 CPP+EI
    let r = marginalFed + marginalState;
    if (w2Gross < SS_WAGE_BASE_2025) r += 0.062;  // SS 下个美元
    r += 0.0145;  // Medicare always
    if (w2Gross > addlMedicareThreshold) r += 0.009;  // Addl Medicare
    return Math.min(r, 0.65);
  })();

  // 1099 marginal
  const i1099MarginalRate = (() => {
    if (isCA) return marginalFed + marginalState + 0.119;  // CA 自雇 CPP 11.9% 简化
    let r = marginalFed + marginalState;
    const remainingSScap = Math.max(0, SS_WAGE_BASE_2025 - w2Gross);
    const seRate = i1099Gross < remainingSScap ? 0.153 : 0.029;  // above cap: Medicare only
    const seEff = seRate * 0.9235 * (1 - 0.5 * marginalFed);  // half SE deductible
    r += seEff;
    return Math.min(r, 0.65);
  })();

  const rentalMarginalRate = Math.min(marginalFed + marginalState, 0.65);
  const ordInvestMarginalRate = Math.min(marginalFed + marginalState, 0.65);

  // LTCG marginal rate · 查 ltCGBrackets
  const ltcgMarginalRate = (() => {
    const brackets = calc.ltCGBrackets || [];
    if (brackets.length === 0) return 0.15;
    const taxable = calc.fedTaxable || 0;
    for (const [thresh, rate] of brackets) {
      if (taxable < thresh) return rate;
    }
    return 0.20;
  })();
  const prefMarginalRate = Math.min(ltcgMarginalRate + marginalState, 0.65);

  // 汇总 · 分源计算 impliedNet · 再归一到 cashTakeHome
  const rawBreakdown = [
    { label: 'W2 工资', gross: w2Gross, rate: w2MarginalRate, color: C.save },
    { label: isCA ? '自雇/合同工' : '1099 自雇', gross: i1099Gross, rate: i1099MarginalRate, color: C.info },
    { label: '房租净', gross: rentalGross, rate: rentalMarginalRate, color: C.warn },
    { label: '投资 (优惠)', gross: prefIncome, rate: prefMarginalRate, color: '#7A8B9E' },
    { label: '投资 (普通)', gross: ordInvestIncome, rate: ordInvestMarginalRate, color: C.ink2 },
  ].filter(s => s.gross > 0).map(s => ({
    ...s,
    impliedNet: s.gross * (1 - s.rate),
  }));
  const impliedTotal = rawBreakdown.reduce((sum, s) => sum + s.impliedNet, 0);
  // 归一到月到手 · 让 4-5 项之和恰好等于 monthlyCash · 保持显示一致
  const sourceBreakdown = rawBreakdown.map(s => ({
    ...s,
    monthlyShare: impliedTotal > 0 ? monthlyCash * s.impliedNet / impliedTotal : 0,
    takehomePct: impliedTotal > 0 ? s.impliedNet / impliedTotal : 0,
  }));
  ...
  const taxItems = [
    ...
    // v131: 州级 PFML · 单独显示 · 紫灰
    { label: calc.pfmlRule?.label || 'PFML', value: calc.statePFML || 0, color: '#A990B5' },
  ].filter(x => x.value > 0);
  ...
  return (
    <div className="rounded-2xl mb-2" style={{
      ...
      <div style={{
        ...
        TAX YEAR 2025
        {calc?._tunerDelta > 0.0001 && (
          <span style={{
            marginLeft: 6, color: C.save, fontWeight: 700,
            letterSpacing: '0.05em',
          }}>
            · 执行到 {fmtPct(calc._tunerPct)} 预览
          </span>
        )}
        {calc?._tunerDelta < -0.0001 && (
          <span style={{
            marginLeft: 6, color: C.pay, fontWeight: 700,
            letterSpacing: '0.05em',
          }}>
            · 倒推到 {fmtPct(calc._tunerPct)} 预览
          </span>
        )}
      </div>
      ...
      <div style={{
        ...
        {/* 当下到手（现金）+ 延税资产 · 守恒：当下到手 + 延税 + 总税负 + premium = 总收入 */}
        <div style={{ textAlign: 'right', padding: '0 4px' }}>
          ...
          {/* v131: 月到手 · v132 加详情按钮 · 可展开看分源和分期
              v137: 显示跟随 payFreq · 4 格可联动切换 */}
          <button
            onClick={() => setTakehomeDetailOpen(v => !v)}
            style={{
              background: 'transparent', border: 'none',
              padding: '2px 0', marginTop: 2,
              cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 3,
              fontSize: 9, color: C.mute, fontFamily: F_NUM, fontWeight: 600,
              letterSpacing: '-0.01em',
            }}
          >
            ≈ ${fmt(Math.round(currentFreq.value))}/{currentFreq.unit}
            <span style={{
              fontSize: 8, color: C.muteLite,
              transform: takehomeDetailOpen ? 'rotate(180deg)' : 'none',
              transition: 'transform 0.2s',
            }}>▾</span>
          </button>
      ...
      {takehomeDetailOpen && (
        <div style={{
          ...
          <div style={{
            fontSize: 9, color: C.mute, fontWeight: 700,
            letterSpacing: '0.08em', textTransform: 'uppercase',
            marginBottom: 6,
            display: 'flex', justifyContent: 'space-between', alignItems: 'baseline',
          }}>
            <span>每期到手 · 按发薪频率</span>
            <span style={{
              fontSize: 7, color: C.muteLite, fontWeight: 500,
          ...
          <div style={{
            ...
            {[
              { key: 'weekly',      label: '每周 w',  value: weeklyCash,      note: '52 期' },
              { key: 'biweekly',    label: '双周 bw', value: biweeklyCash,    note: '26 期' },
              { key: 'semimonthly', label: '月半 sm', value: semimonthlyCash, note: '24 期' },
              { key: 'monthly',     label: '每月 m',  value: monthlyCash,     note: '12 期' },
            ].map(p => {
              const active = payFreq === p.key;
              return (
                <button
                  key={p.key}
                  onClick={() => setPayFreq(p.key)}
                  style={{
                    padding: '6px 4px',
                    background: active ? C.info : C.card,
                    borderRadius: 5,
                    border: `1px solid ${active ? C.info : C.lineLite}`,
                    textAlign: 'center',
                    cursor: 'pointer',
                    transition: 'all 0.15s',
                    fontFamily: 'inherit',
                ...
                >
                  <div style={{
                    fontSize: 8,
                    color: active ? 'rgba(255,255,255,0.72)' : C.muteLite,
          ...
          {sourceBreakdown.length > 1 && (
            <>
              ...
              {sourceBreakdown.map(s => (
                <div key={s.label} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ color: C.ink2, whiteSpace: 'nowrap' }}>{s.label}</span>
                    <span style={{
                      fontSize: 8, color: C.muteLite,
                      fontFamily: F_MONO, letterSpacing: '-0.03em',
                    }}>
                      扣 {Math.round(s.rate * 100)}%
                    </span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexShrink: 0 }}>
                    <span style={{ fontSize: 8, color: C.muteLite, fontFamily: F_MONO }}>
                      {Math.round(s.takehomePct * 100)}%
                    </span>
                    <span style={{ fontFamily: F_NUM, fontWeight: 700, color: C.ink, fontSize: 11 }}>
                      ${fmt(Math.round(s.monthlyShare))}
                    </span>
                    <span style={{ color: C.muteLite, fontWeight: 500, fontSize: 8 }}>
                      /月
                    </span>
                  </div>
                </div>
              ...
              <div style={{
                marginTop: 6, fontSize: 8, color: C.muteLite, fontFamily: F_BODY,
                lineHeight: 1.4, paddingLeft: 12,
              }}>
                § 每项税率 = 该类型「下一美元」边际（fed + state + FICA / SE / LTCG 相应）
                <br/>§ 分源之和已归一到月到手 · 比例来自各源税后 implied net
              </div>
            </>
          )}
          {/* v140: 仿真 paystub 彩蛋 · 小字 · 懂 ADP 的人会点 */}
          <div style={{
            marginTop: 10, paddingTop: 8,
            borderTop: `1px dotted ${C.lineLite}`,
            textAlign: 'right',
          }}>
            <button
      ...
      {/* v140: Paystub 模态框 · ADP 风格 · 纯娱乐 */}
      {paystubOpen && (
        <PaystubModal
          calc={calc}
          payFreq={payFreq}
          isCA={isCA}
          onClose={() => setPaystubOpen(false)}
        />
      )}

      {/* v63: 守恒等式小字 · 让用户看清所有数字之和 = 总收入
          v131: 加 premium 桶 · grossWages = 到手 + 延税 + 税负 + 保险费 */}
      <div style={{
        marginTop: 8,
        fontSize: 8, color: C.muteLite, fontFamily: F_MONO,
        ...
        {(calc.premiumSpending || 0) > 0 && (
          <>
            <span style={{ margin: '0 3px', opacity: 0.5 }}>+</span>
            <span style={{ color: C.ink2 }}>保险 ${fmt(calc.premiumSpending)}</span>
          </>
        )}
        <span style={{ margin: '0 3px', opacity: 0.5 }}>+</span>
...
const SavingsTuner = ({ opps, calc, inputs, pct, setPct, baselinePct = 0, mode = 'priority', setMode }) => {
  // v137: 国家推导 · 用于 priority 模式描述本地化
  const isCA = calc?._country === 'CA';
  // v137: 模式描述 CA 化 · HSA/401(k)/Mega 是美国账户 · CA 用 RRSP/FHSA/TFSA
  const modeDescLocal = {
    proportional: '全部同时按比例',
    priority:     isCA ? 'RRSP → FHSA → TFSA' : 'HSA → 401(k) → Mega',
    efficient:    '每块钱省最多税先',
  };
  // 把机会分两类：
  ...
  //     默认全部执行（当作 bonus）
  // v125: 排除 itemizeDone · 它是「已优于 standard 的状态播报」· saving/contrib 已内置在 calc 里
  //       不是可操作项 · 进了会显示「○ 未开始 · —」让用户困惑
  ...
  // v120: 判定改为"偏离 baseline"（baseline 处等于老行为的 pct=0 · 无信息增量）
  // v121: step 0.1 · 用 epsilon 避免 float 精度问题
  // v127: autoOpen 永远 true · 仪表板不再因为"在 baseline"而自动收起
  //       避免用户拖回 baseline 时仪表板"突然消失"的不连续感
  //       用户仍可手动点「收起」按钮关闭
  ...
  // v120: 显示条件加一条 · 若已存供款 >= $500 · 即使没多少 gap 可做 · 也要显示以便预览倒推
  const alreadyTotal = (inputs?.k401 || 0) + (inputs?.hsa || 0) + (inputs?.solo401k || 0) + (inputs?.hsa2 || 0);
  if (totalPossibleSaving < 500 && alreadyTotal < 500) return null;

  // 滑块的当前值
  // v46: 0% = 啥都不做 · 所有机会（包括免费型）按 scale 缩放
  // v120: 语义变更 · scale 相对 baseline 位置
  //   · pct === baselinePct → scale = 0（当前状态 · 与 Hero 相等）
  //   · pct === 100         → scale = 1（满档）
  //   · pct === 0           → scale < 0（倒推 · 假设撤回所有已存）
  //   · 老行为兼容：baselinePct === 0 时退化为 pct/100
  // v121: step 0.1 · pct/baselinePct 可能有小数 · isBaseline 用小 epsilon 避免 float 比较 bug
  const denom = 100 - baselinePct;
  const scale = denom > 0 ? (pct - baselinePct) / denom : 0;
  const isReverse = scale < -0.0001; // 倒推模式 · UI 用不同色标
  const isBaseline = Math.abs(scale) < 0.0001; // 在 baseline 处 · 什么都不显示增量
  // v122-123: 分配模式由 prop 传入 · 用户可在 toggle 切换
  //   · proportional: 每个 opp 独立填 scale × cap（v120-121 行为）
  //   · priority:     HSA → 401(k) → Mega 顺序填满（v122 默认）
  //   · efficient:    按 saving/contrib 比率排序 · 每块钱省最多税先填
  const allocation = allocateByMode(contribOpps, scale, mode);
  const chosenContribSaving = allocation.totalSaving;
  const chosenContribAmt = allocation.totalContrib;
  // Free opps 不按优先级 · 仍用 proportional · 负 scale 时归零
  const freeScale = Math.max(0, scale);
  const chosenFreeSaving = Math.round(freeSaving * freeScale);
  ...
  // v129: 删掉预设档位声明 · UI 不再用快捷按钮
  //   理由：presets 占空间 · 视觉噪声大 · 和 bubble/slider/snap 功能重复
  //   保留 magnetic snap（baseline 回归）和 slider 直接拖拽作为交互手段
  ...
  return (
    <div className="rounded-xl mb-2 px-3 py-3" style={{
      ...
      {/* v128: 固定 thumb 尺寸跨浏览器 · bubble 位置公式才能精准对齐
              原生 range input 的 thumb 各浏览器默认宽度不同（desktop ~16 · iOS ~28）
              显式指定 18×18 后 thumb 中心 = 9 + pct × (w-18)/100 · 公式可闭合 */}
      <style dangerouslySetInnerHTML={{ __html: `
        .tp-tuner-slider { -webkit-appearance: none; appearance: none; }
        .tp-tuner-slider::-webkit-slider-thumb {
          -webkit-appearance: none; appearance: none;
          width: 18px; height: 18px; border-radius: 50%;
          background: #FFFFFF; border: 1px solid ${C.ink};
          cursor: pointer; margin-top: -6px;
          box-shadow: 0 1px 2px rgba(0,0,0,0.08);
        }
        .tp-tuner-slider::-moz-range-thumb {
          width: 18px; height: 18px; border-radius: 50%;
          background: #FFFFFF; border: 1px solid ${C.ink};
          cursor: pointer;
          box-shadow: 0 1px 2px rgba(0,0,0,0.08);
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{
            ...
            {baselinePct > 0
              ? `0% = 全撤 · ${fmtPct(baselinePct)} = 当前 · 100% = 满档`
              : '0% = 顶部当下到手 · 100% = 全部执行后'
            }
          </div>
        </div>
      ...
      {/* v123: 分配模式切换 · 用户选而非系统猜 · 只在有 ≥2 个 contrib opps 时显示（1 个时无意义） */}
      {contribOpps.length >= 2 && setMode && (
        <div style={{
          display: 'flex', alignItems: 'center', gap: 6,
          marginBottom: 8, padding: '0 2px',
          fontSize: 9, fontFamily: F_BODY,
        }}>
          <span style={{ color: C.mute, fontWeight: 600, flexShrink: 0 }}>
            分配方式
          </span>
          <div style={{
            display: 'flex', gap: 0,
            ...
            {ALLOC_MODE_KEYS.map((k, idx) => {
              const m = ALLOC_MODES[k];
              const active = mode === k;
              return (
                <button
                  key={k}
                  ...
                  style={{
                    padding: '3px 8px',
                    background: active ? C.info : 'transparent',
                    color: active ? '#FFF' : C.ink2,
                    border: 'none',
                    borderLeft: idx > 0 ? `1px solid ${C.line}` : 'none',
                    cursor: 'pointer',
                    fontFamily: F_BODY, fontWeight: active ? 700 : 500,
                    fontSize: 10,
                    letterSpacing: '0.02em',
                    transition: 'all 0.12s',
                  }}
                  title={modeDescLocal[k]}
                >
                  {m.label}
                </button>
              );
          ...
          {/* 当前选中模式的小字描述 · v137 country-aware */}
          <span style={{ color: C.muteLite, fontSize: 8, flexShrink: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            · {modeDescLocal[mode]}
          </span>
        </div>
      )}

      {/* v129: 去掉预设快捷按钮 · 去掉 baseline tick
             原因：视觉噪声太多 · slider + 磁吸 + bubble 里的「当前」提示已够用
             用户仍可自由拖动任意 pct · 磁吸让 baseline 容易命中 · bubble 文字告知是否在 baseline */}

      {/* v121: baseline 提示移出 slider wrapper · 与 bubble 不再共享空间 · 彻底去遮挡 */}
      {baselinePct > 0 && (
        <div style={{
          fontSize: 9, color: C.mute, fontFamily: F_BODY,
          ...
          <span style={{ color: C.save, fontWeight: 700 }}>✓ 你当前 {fmtPct(baselinePct)}</span>
          <span style={{ color: C.muteLite }}>
            · 已存 ${fmt((inputs.k401 || 0) + (inputs.hsa || 0) + (inputs.solo401k || 0))} · 向右再做 · 向左倒推（假设）
          </span>
        </div>
      )}

      {/* 滑块本体
          v121: paddingTop 24 为 bubble 预留位 · bubble top:0 落在 padding 区内 · 绝不遮挡任何文字
          v127: 去掉水平 padding · bubble 用 transform 居中 · 和 thumb 严格对齐
                边缘 0%/100% 用 translateX(0)/(-100%) 防出界
          v128: bubble 位置公式补偿 thumb 半径 9px · 跨 pct 全域精准对齐
          v129: 去掉 baseline tick 虚线 · bubble 在 baseline 时显「当前 · X%」替代 */}
      <div style={{ padding: '24px 0 0', position: 'relative' }}>
        {/* v119: 拇指上方浮动百分比 bubble
            v120: 偏离 baseline 时显示方向箭头色
            v121: 固定 top:0 在 padding 区内 · 永不遮挡 label
            v127: 位置公式统一 left:${pct}% + translate
            v128: + 补偿 thumb 半径偏移 · 左闭合右闭合
            v129: baseline 时 bubble 文字改「当前 X%」· 自动变宽 · 替代虚线 tick 的作用 */}
        {(() => {
          // thumb 强制 18×18 后 · 中心位置 = ${pct}% + (9 - pct × 0.18)px
          const thumbOffset = (9 - pct * 0.18).toFixed(2);
          const bubbleWidth = isBaseline ? 72 : 44;
          return (
            <div style={{
              position: 'absolute',
              top: 0,
              ...
              width: bubbleWidth,
              textAlign: 'center',
              fontSize: 10, fontFamily: F_NUM, fontWeight: 700,
              color: isReverse ? C.pay : (isBaseline ? '#FFF' : C.save),
              background: isBaseline ? C.info : C.card,
              border: `1px solid ${isReverse ? C.pay : (isBaseline ? C.info : C.save)}`,
              borderRadius: 4,
              padding: '1px 0',
              pointerEvents: 'none',
              transition: 'left 0.1s, width 0.2s, background 0.2s, color 0.15s, border-color 0.15s',
              whiteSpace: 'nowrap',
              letterSpacing: isBaseline ? '0.04em' : 0,
            }}>
              {isBaseline ? `当前 · ${fmtPct(pct)}` : fmtPct(pct)}
            </div>
          );
        })()}
        <input
          className="tp-tuner-slider"
          type="range"
          min="0"
          max="100"
          step="0.1"
          value={pct}
          onChange={(e) => {
            // v126: 磁吸 baseline · 拖到 ±0.5% 内自动卡到精确 baselinePct
            //       用户无需纠结怎么拖回精准 "当前"（例 22.2% 这种 1 位小数）
            //       阈值 0.5 · 约等于 5 个 0.1% step · 覆盖 baseline 前后 5 步
            //       v129: tick 已删 · 磁吸成为 baseline 回归的唯一机制 · 阈值不变
            const raw = Number(e.target.value);
            if (baselinePct > 0 && Math.abs(raw - baselinePct) < 0.5) {
              setPct(baselinePct);
            } else {
              setPct(raw);
            }
          ...
          style={{
            ...
            appearance: 'none',
            borderRadius: 3,
            // v120: 三段式 gradient · baseline 两侧不同色
            //   · baseline 左到 pct  = 红（倒推段）
            //   · baseline 右到 pct  = 绿（前进段）
            //   · 其余灰
            background: (() => {
              const L = Math.min(pct, baselinePct);
              const R = Math.max(pct, baselinePct);
              const activeColor = pct > baselinePct ? C.save : C.pay;
              if (baselinePct === 0) {
                return `linear-gradient(to right, ${C.save} 0%, ${C.save} ${pct}%, ${C.lineLite} ${pct}%, ${C.lineLite} 100%)`;
              }
              return `linear-gradient(to right, ${C.lineLite} 0%, ${C.lineLite} ${L}%, ${activeColor} ${L}%, ${activeColor} ${R}%, ${C.lineLite} ${R}%, ${C.lineLite} 100%)`;
            })(),
            outline: 'none',
            cursor: 'pointer',
          }}
        ...
        <div style={{
          ...
          <span>{baselinePct > 0 ? '0% · 全撤' : '0% · 啥都不做'}</span>
          <span style={{ fontWeight: 700, color: isReverse ? C.pay : (isBaseline ? C.ink : C.save), fontFamily: F_NUM, fontSize: 11 }}>
            {isBaseline ? '当前' : (isReverse ? `倒推到 ${fmtPct(pct)}` : `执行到 ${fmtPct(pct)}`)}
          </span>
          <span>100% · 满档</span>
        </div>
      </div>
      ...
      {/* 核心对比 · 当下到手 vs 省税
          v127: 去掉 !isBaseline guard · baseline 处也显示（delta=0 · 无突然消失感） */}
      <div style={{
        marginTop: 12,
        padding: '14px 16px',
        background: isBaseline ? C.cardAlt : (isReverse ? C.payBg : C.saveBg),
        border: `1px solid ${isBaseline ? C.line : (isReverse ? C.pay + '55' : C.save + '55')}`,
        borderRadius: 10,
        display: 'grid',
        ...
        <div style={{
          ...
          <div style={{
            fontSize: 9,
            color: isBaseline ? C.mute : (isReverse ? C.pay : C.save),
            fontWeight: 700,
            letterSpacing: '0.1em', textTransform: 'uppercase',
          ...
          <div style={{
            ...
            color: isBaseline ? C.ink : (newCash >= currentCash ? C.save : C.pay),
          ...
          <div style={{
            fontSize: 10, fontFamily: F_BODY, fontWeight: 600,
            color: isBaseline ? C.mute : (cashDelta >= 0 ? C.save : C.pay),
            marginTop: 3,
          }}>
            {isBaseline ? '±$0 · 在当前' : (
              <>
        ...
        {/* 中：虚线竖线 */}
        <div style={{
          borderLeft: `1.5px dashed ${isReverse ? C.pay + '66' : C.save + '66'}`,
          alignSelf: 'stretch',
        }} />

        {/* 右：少交的税 · 靠中间 · v120 倒推时切换为"多交的税" */}
        <div style={{
          textAlign: 'left',
          paddingLeft: 16,
          display: 'flex', flexDirection: 'column',
          ...
          <div style={{
            ...
            {isBaseline ? '少交的税' : (isReverse ? '多交的税' : '少交的税')}
          </div>
          <div style={{
            fontFamily: F_NUM, fontSize: 26, fontWeight: 800,
            color: isBaseline ? C.ink : (isReverse ? C.pay : C.save),
            letterSpacing: '-0.03em', lineHeight: 1,
          }}>
            {isBaseline ? '$0' : `${isReverse ? '+' : '−'}$${fmt(Math.abs(totalTaxSaved))}`}
          </div>
          <div style={{
            ...
            <span style={{
              ...
              {calc?.totalTax > 0 ? Math.round(Math.abs(totalTaxSaved) / calc.totalTax * 100) : 0}%
            </span>
          </div>
          <div style={{
            fontSize: 10, fontFamily: F_BODY, fontWeight: 600,
            color: chosenContribAmt > 0 ? C.info : (chosenContribAmt < 0 ? C.pay : C.mute),
            marginTop: 3,
          }}>
            {chosenContribAmt > 0 && (
            ...
            {chosenContribAmt < 0 && (
              <>−${fmt(Math.abs(chosenContribAmt))} 延税资产（撤回）</>
            )}
            {chosenContribAmt === 0 && (
              <>{isBaseline ? '在当前 · 无变动' : '无需供款 · 纯省'}</>
            )}
          </div>
        </div>
      </div>
      ...
      {showDetails && (
        <div style={{
          ...
          {/* 免费的（不需投入现金）· v46: 也跟滑块比例
              v120: 倒推时隐藏整段（免费机会不存在"撤回"概念）
              v127: baseline 处 freeScale=0 · 隐藏整段（0 值显示无意义） */}
          {freeOpps.length > 0 && !isReverse && !isBaseline && (
            <>
              <div style={{ fontSize: 9, color: C.mute, marginBottom: 3 }}>
                ◆ 免费机会（不投现金 · 执行 {fmtPct(freeScale * 100)}）
              </div>
              {freeOpps.map((o, idx) => {
                const saving = Math.round((o.saving || 0) * freeScale);
                return (
                  <div key={`f${idx}`} style={{
                    display: 'flex', justifyContent: 'space-between',
                    padding: '2px 0',
                    ...
                    <span>· {o.title}</span>
                    <span style={{ fontFamily: F_NUM, color: saving > 0 ? C.save : C.muteLite, fontWeight: 600 }}>
                      {saving > 0 ? `−$${fmt(saving)}` : '—'}
                    </span>
                  </div>
                );
            ...
            <>
              <div style={{
                ...
                {isBaseline
                  ? '在当前 · 未动'
                  : (isReverse
                      ? `倒推到 ${fmtPct(pct)} · 撤回 ${fmtPct(Math.abs(scale) * 100)}`
                      : `执行到 ${fmtPct(pct)}`)}
                ）
              </div>
              {/* v119: 已存供款 · 展示在 action list 顶部（已发生 · 不受 slider 影响）
                  v122: 按优先级排序展示 · HSA 第一 · 然后 401(k) · 然后 Solo */}
              {(inputs.hsa || 0) > 0 && (
                <div style={{
                  display: 'grid',
                  ...
                  <span style={{ fontSize: 9, fontWeight: 600 }}>✓ 已存 HSA</span>
                  <span style={{
              ...
              {(inputs.k401 || 0) > 0 && (
                <div style={{
                  ...
                  <span style={{ fontSize: 9, fontWeight: 600 }}>✓ 已存 401(k)</span>
                  <span style={{
                    fontSize: 9, textAlign: 'right', fontFamily: F_NUM,
                    fontWeight: 600,
                  }}>
              ...
              {(inputs.solo401k || 0) > 0 && (
                <div style={{
                  ...
                  <span style={{ fontSize: 9, fontWeight: 600 }}>✓ 已存 Solo 401(k)</span>
                  <span style={{
                    fontSize: 9, textAlign: 'right', fontFamily: F_NUM,
                    fontWeight: 600,
                  }}>
                    ${fmt(inputs.solo401k || 0)}
                  </span>
                  <span style={{
                    fontSize: 9, textAlign: 'right', fontFamily: F_NUM,
                    color: C.muteLite,
              ...
              {/* v122: 按优先级顺序渲染分配 · 显示 ① ② ③ 编号 + 填充状态徽章
                   · 正向 scale: ✓ 满 / ↗ 填 X% / ○ 未开始
                   · 倒推 scale: 所有项都是按比例撤 · 状态统一"撤"
                   · 优先级用 CN 圆圈数字（① ② ③ ④ ⑤ ⑥ ⑦ ⑧ ⑨）· 10+ 用数字括号 (10) */}
              {allocation.allocs.map((a, idx) => {
                const o = a.opp;
                const absContrib = Math.abs(a.contrib);
                const absSaving = Math.abs(a.saving);
                const rankMark = idx < 9
                  ? ['①','②','③','④','⑤','⑥','⑦','⑧','⑨'][idx]
                  : `(${idx + 1})`;
                // 状态徽章 · 仅正向 scale 显示
                ...
                if (!isReverse) {
                  if (a.fillFrac >= 0.999) {
                    statusBadge = { text: '✓ 满', color: C.save };
                  } else if (a.fillFrac > 0.001) {
                    statusBadge = { text: `↗ ${fmtPct(a.fillFrac * 100)}`, color: C.save };
                  } else {
                    statusBadge = { text: '○ 未开始', color: C.muteLite };
                  }
                }
                const isActive = absContrib > 0;
                return (
                  <div key={`c${idx}`} style={{
                    display: 'grid',
                    gridTemplateColumns: '18px 1fr 80px 80px',
                    gap: 6,
                    padding: '3px 0',
                    fontSize: 10,
                    color: isActive ? C.ink2 : C.muteLite,
                    alignItems: 'baseline',
                  }}>
                    <span style={{
                      fontSize: 10, fontFamily: F_NUM, fontWeight: 700,
                      color: isActive ? (isReverse ? C.pay : C.save) : C.muteLite,
                      textAlign: 'center',
                    }}>
                      {rankMark}
                    </span>
                    <span style={{ fontSize: 9, display: 'flex', alignItems: 'baseline', gap: 4, flexWrap: 'wrap' }}>
                      <span>{o.title}</span>
                      {statusBadge && (
                        <span style={{
                    ...
                    <span style={{
                      fontSize: 9, textAlign: 'right', fontFamily: F_NUM,
                      color: isReverse ? C.pay : (isActive ? C.info : C.muteLite),
                    }}>
                      {absContrib > 0
                        ? `${isReverse ? '撤' : '存'} $${fmt(absContrib)}`
                        : '—'}
                    </span>
                    <span style={{
                      fontSize: 9, textAlign: 'right', fontFamily: F_NUM,
                      color: isReverse ? C.pay : (isActive ? C.save : C.muteLite),
                      fontWeight: 600,
                    }}>
                      {absSaving > 0
                        ? `${isReverse ? '+' : '−'}$${fmt(absSaving)}`
                        : '—'}
                    </span>
                  </div>
                );
              ...
              <div style={{
                ...
                <span></span>
                <span style={{ color: isBaseline ? C.mute : C.ink2 }}>合计</span>
                <span style={{ textAlign: 'right', fontFamily: F_NUM, color: isBaseline ? C.muteLite : (isReverse ? C.pay : C.info) }}>
                  {isBaseline ? '—' : `${isReverse ? '−' : ''}$${fmt(Math.abs(chosenContribAmt))}`}
                </span>
                <span style={{ textAlign: 'right', fontFamily: F_NUM, color: isBaseline ? C.muteLite : (isReverse ? C.pay : C.save) }}>
                  {isBaseline ? '—' : `${isReverse ? '+' : '−'}$${fmt(Math.abs(chosenContribSaving))}`}
                </span>
              </div>
              {/* v122-123: 小字说明分配逻辑 · 按 mode 不同措辞 */}
              {!isReverse && allocation.allocs.length > 1 && (
                <div style={{
                  fontSize: 8, color: C.muteLite, marginTop: 4,
                  fontFamily: F_BODY, lineHeight: 1.4,
                }}>
                  {mode === 'proportional' && '※ 平均分配 · 每个机会同步按比例推进 · 合计与其他模式相同'}
                  {mode === 'priority' && `※ 按实用优先级顺序填（${isCA ? 'RRSP → FHSA → TFSA' : 'HSA → 401(k) → Mega'} 等）· 同样美金省更多税`}
                  {mode === 'efficient' && '※ 按"每块钱省最多税"排序 · 效率最高的先填满'}
                </div>
              )}
              {isReverse && allocation.allocs.length > 1 && (
                <div style={{
...
const ScenarioCompare = ({ inputs, calc }) => {
  ...
  const usScenarios = [
    ...
    { id: 'max401k', label: '存满 W2 401(k)', icon: '↗', group: '策略', patch: (i) => ({ k401: (i.filingStatus === 'MFJ' && (i.spouseW2 || 0) > 0) ? K401_LIMIT_2025 * 2 : K401_LIMIT_2025 }) },
      ...
      <div className="px-4 py-2" style={{ borderBottom: `1px solid ${C.lineLite}` }}>
        ...
        {!currentWorkHasConv && (inputs.workState && inputs.workState !== inputs.state) && (
          <div style={{
            ...
            <b>¶ </b>搬家场景假设 100% 远程 · 只交新居住{isCA ? '省' : '州'}税
          </div>
        )}
      </div>
...
const GLOSSARY = {
  ...
  // ═══ 自雇 HST / GST · 3 条 ═══
  HST: {
    full: 'Harmonized / Goods & Services Tax',
    cn: '消费税',
    desc: '加拿大的**销售税** · 跟所得税完全独立的系统。自雇 / 公司开票收税 · 季度/年度填表交 CRA。**省率**：ON/NB/NL/NS/PE 13-15% HST（联邦省合并）· BC/AB/SK/MB GST 5% (+ PST 分开) · QC GST 5% + QST 9.975%。**$30K 滚动 4 季门槛**：过了必须注册。**不是你自己的钱** · 是代 CRA 收 · 中间可以抵 ITC（进项）。',
    cta: '过 $30K 立刻 CRA My Business Account 注册 · 不要拖。',
  },
  ITC: {
    full: 'Input Tax Credit · 进项税抵扣',
    cn: '进项抵扣',
    desc: '你付业务费用时**交的** HST/GST · 可以**抵扣你收的** HST/GST。例：ON 律师收客户 $113K（含 $13K HST）· 付业务费 $33.9K（含 $3.9K HST）· 净交 CRA = $13K - $3.9K = $9.1K。**能全抵**：办公租金 · 设备 · 软件 · 营销 · 差旅 · 专业服务。**按比例抵**：手机 · 宽带 · Home Office（按面积）· 车辆（按业务里程）。**只 50%**：餐饮招待 · Meals & Entertainment。',
    cta: '保留所有 receipt · GST/HST 号要在发票上 · 6 年审计追溯期。',
  },
  QuickMethod: {
    full: 'Quick Method · 简化征收法',
    cn: 'HST 简化法',
    desc: '年收 < $400K 自雇小户可选的简化选项。**不追 ITC** · 直接按固定 % 交。**服务业**：ON 8.8% · BC/AB 3.6% · NB/NL/NS/PE 10.5%。**零售业**：税率稍低。**怎么算**：开票照样收 13%/15%/5% HST · 但实际按低税率交 · 差价合法留下。**优点**：免记账 · 差价留自己。**缺点**：大笔设备采购年亏（放弃 ITC）。**适合**：咨询 / 写作 / 设计 / 网红 / 低费用高毛利服务业。**要主动 elect**：填 GST74 · 每财年 1/1 生效。',
    cta: '每年 1 月前决定本年用不用 · 填 GST74 表。',
  },
};
...
const MYTHS_CA = [
  ...
  // ═══ 自雇 HST / GST / ITC · 3 条 ═══
  {
    q: '自雇收入多少要注册 HST？', cat: 'biz', hot: true,
    tags: ['selfemp', 'hst', 'gst', 'threshold', 'registration'],
    a: '**滚动 4 季度 > $30,000** 强制注册。注意不是**年度 calendar year**而是**任意连续 4 个季度**。例如你 2024 Q4 + 2025 Q1-Q3 加起来过 $30K · 就要从 **2025 Q4 开始的下一次开票**起注册收税 + 30 天内正式申请。**低于 $30K 叫 Small Supplier** · 可以不收不交 · 但也**不能抵 ITC**。**跨省客户**：按**客户所在地**税率收（Place of Supply 规则）· AB 客户只收 5% GST · ON 客户收 13% HST。**漏注册后果**：CRA 倒查最长 4 年 · 补 + 罚金（按月 1% 复利）· 过了门槛还继续当 small supplier 开票就是偷税。',
    gloss: { 'HST': '联邦+省合并消费税 · ON/NB/NL/NS/PE', 'GST': '联邦 5% 消费税 · 全国基础', 'Small Supplier': '小型供应商 · $30K 以下免注册免收 HST' },
  },
  {
    q: '餐饮招待可以 100% 抵 ITC 和业务费用吗？', cat: 'biz', hot: true,
    tags: ['selfemp', 'itc', 'expenses', 'meals', 'entertainment'],
    a: '**不能 · 只 50%**。加拿大税法 §67.1 限制：餐饮 · 饮料 · 娱乐（比如招待客户吃饭 / 请客 / 员工聚餐）**所得税扣除 50%** · 对应的 **ITC 也只能抵 50%**。例外情况 **可以 100% 抵**：（1）公司年度 6 次以下全员聚餐（注意是 6 次/年上限）（2）长途出差 12+ 小时 · 自己的餐饮（3）交通从业者（卡车司机）的餐饮（4）客户要求 + 发票开给客户的 reimbursable 餐饮。**怎么记账**：发票上原金额全记 · 报税时按 50% 扣 · T2125 L8523 行。**坑**：用 credit card 挂 auto-categorize 为 "meals" · 报税直接全抵 · 会被 audit。',
    gloss: { 'ITC': '进项抵扣 · 付出去 HST 可以抵扣收的 HST', 'T2125': '自雇业务活动表 · 附 T1 提交' },
  },
  {
    q: 'Quick Method 值得用吗？', cat: 'biz',
    tags: ['selfemp', 'quick-method', 'hst', 'gst', 'strategy'],
    a: '**低费用 · 高毛利的服务业 · 大部分情况值得**。**适合**：咨询 / 写作 / 设计 / 网红 / 翻译 / 摄影 · 费用率 < 25% 的。**不适合**：电商（大进货）· 零售（高进货）· 餐饮（高成本）· 每年有大笔设备采购。**算法对比**（ON 律师 · 年收 $100K · 费用 $10K）：常规法净交 $13K - $1.3K = $11.7K。Quick Method = $100K × 8.8% = $8.8K。**省 $2.9K**。**前提**：年营收 < $400K · 不跨大量省份 · 主动填 **GST74** election · 每年 1/1 生效 · 元旦后改变不了。**首次注册**可第一年就 elect。**最容易忽略**：用 Quick Method 还是开票收 13%/15%/5% 全额 HST（不是按 8.8% 开票）· 只是交 CRA 时按 quick rate · 差价合法留下。',
    gloss: { 'Quick Method': 'HST 简化征收法 · 不追 ITC · 按固定 % 交', 'GST74': 'Election to Use Quick Method 申请表' },
  },
];
...
// v135: Myth body renderer · 解析原始文本为结构化排版
//   支持：
//     \n          → 换行
//     \n\n        → 段落间距
//     \n·         → bullet point（缩进 · dot 靠左）
//     \n①②③      → 编号列表（数字符靠左）
//     **text**    → 加粗强调（term 高亮）
//     [TERM]      → 术语小徽章（可选 · 后续加）
//   输出：每行按类型渲染成不同的 visual 块 · 消除 wall of text
const parseInlineBold = (line) => {
  // Returns array of { text, bold } segments
  const parts = [];
  let remaining = line;
  let iter = 0;
  while (remaining && iter++ < 50) {
    const m = remaining.match(/\*\*([^*]+)\*\*/);
    if (!m) {
      parts.push({ text: remaining, bold: false });
      break;
    }
    if (m.index > 0) parts.push({ text: remaining.slice(0, m.index), bold: false });
...
const MythBody = ({ text, accentColor }) => {
  if (!text) return null;
  ...
  while (i < lines.length) {
    ...
    if (bulletMatch) {
      ...
      blocks.push({ type: 'number', marker: numberMatch[1], content: numberMatch[2] });
    } else {
  ...
  return (
    <div style={{
      ...
      {blocks.map((b, idx) => {
        ...
        const parts = parseInlineBold(b.content);
        const renderInline = parts.map((p, k) => (
          p.bold
            ? <b key={k} style={{ color: C.ink, fontWeight: 700 }}>{p.text}</b>
        ...
        if (b.type === 'bullet') {
          return (
            <div key={idx} style={{
              ...
              <span style={{
                color: accentColor || C.mute,
                fontWeight: 700, flexShrink: 0,
                fontFamily: F_MONO, fontSize: 11,
                lineHeight: '22px',
              }}>·</span>
              <div style={{ flex: 1, minWidth: 0 }}>{renderInline}</div>
            </div>
          );
        }
        if (b.type === 'number') {
          return (
            <div key={idx} style={{
              ...
              <span style={{
                color: C.ink, fontWeight: 700, flexShrink: 0,
                fontFamily: F_NUM, fontSize: 13,
                lineHeight: '22px',
              }}>{b.marker}</span>
              <div style={{ flex: 1, minWidth: 0 }}>{renderInline}</div>
            </div>
          ...
          >
            <div style={{
              ...
              <div style={{
                ...
                <span style={{
                  fontSize: 9, color: C.ink, fontFamily: F_BODY,
                  ...
                  {MYTH_CAT_LABEL[open.cat]?.label || '误区'}
                </span>
                {open.hot && (
                  <span style={{
                    fontSize: 9, color: '#8B6914', fontFamily: F_BODY,
                ...
                <span style={{
                  fontSize: 10, color: C.muteLite, fontFamily: F_MONO,
                  letterSpacing: '0.04em',
                }}>
                  {openIdx + 1} / {mythsData.length}
                </span>
              </div>
              <button
                onClick={() => setOpenIdx(null)}
            ...
            <div style={{
              ...
              <span style={{
                fontFamily: F_MONO, fontSize: 12, fontWeight: 700,
                color: C.pay, background: C.payBg || '#FDE8E0',
                border: `1px solid ${C.pay}33`,
                borderRadius: 4, padding: '2px 7px',
                flexShrink: 0, marginTop: 3,
                letterSpacing: '0.02em',
              }}>Q</span>
              <span style={{ flex: 1 }}>{open.q}</span>
            </div>
            <MythBody text={open.a} accentColor={openColor.border} />
            {/* v57: 术语解释 */}
    ...
    <div className="mb-3">
      ...
      {calc.stateTax > 0 && (
        <FormulaBlock title={`④ ${STATE_BRACKETS[inputs.state]?.label || inputs.state} 州税 ${stateRuleLabel}`} desc={`州级累进`}>
          ...
          {/* v116: MFJ 双工作州 · 配偶单独一块 */}
          {calc.spouseWorkStateDetails && (
            <>
              <div style={{ paddingTop: 8, fontSize: 10, color: C.mute, fontFamily: F_BODY, fontWeight: 600, letterSpacing: '0.05em' }}>
                跨州工作调整（配偶）：
              </div>
              <FormulaRow
                label={`配偶的 W2`}
                value={inputs.spouseW2 || 0}
                indent
              />
              <FormulaRow
                label={`× 工作州来源比例 ${Math.round(calc.spouseWorkStateDetails.effectivePortion * 100)}%`}
                value={calc.spouseWorkStateDetails.sourceW2}
                op="×"
                indent
              />
              {calc.spouseWorkStateDetails.hasConvenience && calc.spouseWorkStateDetails.daysPortion < 1 && (
                <div style={{ fontSize: 10, color: C.pay, fontFamily: F_BODY, paddingLeft: 12, lineHeight: 1.5, marginTop: 2 }}>
                  † {calc.spouseWorkStateDetails.name} Convenience 规则：远程天数也计为该州来源
                </div>
              )}
              <FormulaRow label={`+ ${calc.spouseWorkStateDetails.name} 非居民税`} value={calc.spouseWorkStateDetails.rawTax} op="+" indent />
            </>
          )}
          {(calc.workStateDetails || calc.spouseWorkStateDetails) && (
            <>
              <FormulaRow label={`− ${inputs.state} 抵免 (最多抵到工作州已交 · 合并)`} value={-calc.crossStateCredit} op="−" indent />
              <FormulaRow label={`净 ${inputs.state} 居民税`} value={calc.residentStateTax - calc.crossStateCredit} op="=" indent />
              <FormulaRow label="合计州税 (多州加总)" value={calc.stateTax} op="=" bold dashed />
            </>
          )}
          {calc.isReciprocal && (
...
// ═══════════════════════════════════════════════════════════
//  v140: 内测版 changelog · 用户向语言（非技术细节）
//  自 v119（本次迭代起点）至今 · 按主题分组
// ═══════════════════════════════════════════════════════════
...
const CHANGELOG_USER_FACING = [
  {
    ...
    color: '#B5351E',
    items: [
      '加上医保 / 牙科 / 视力这些预税扣款 · 你的 AGI 会正确往下走',
      '自动识别 9 个州的 PFML / SDI（NY PFL · NJ FLI · MA / WA / RI / CT / OR / CO / MD）· 按工资自动扣',
      '总税负条显示分类型：联邦 · 州 · FICA · SE · PFML',
    ],
  },
  {
    theme: '每月到手更直观',
    color: '#0F7C4A',
    items: [
      'Hero 上方多了「≈ $X/月」· 一眼看到每月现金',
      '点「▾」展开看 4 种发薪频率（每周 / 双周 / 月半 / 每月）· 可以点切换',
      '切换后顶部金额自动同步 · 想看哪种都行',
      '分源月到手精算：W2 / 1099 / 房租 / 投资 分别用自己类型的「下一美元」边际算',
      '拉省税调节器 · 所有数字都跟着动（到手 · 每期 · 边际率）',
    ],
  },
  {
    theme: '省税调节器更好用',
    color: '#1F4FA0',
    items: [
      '3 种分配方式：平均（比例） · 实用（优先级顺序填） · 最优（每块钱省最多税先）',
      '401(k) / HSA 已存金额的滑块旁边显示「再存 +$X」· 不会再搞混「已存还是新存」',
      '新 YTD 估算器：不记得已存多少？填「每张工资 6%」或「每期 $200」· 自动反推',
      '调节器 bubble 颜色从黑换成蓝 · 更柔和',
    ],
  },
  {
    theme: '加拿大版补齐了',
    color: '#E0B977',
    items: [
      'Wizard 第 3 步：州 → 省 · 地税不再写「学校税」（加拿大没这个分项）',
      '出租房：折旧换成 CCA 说明（Class 1 · 4% DB · 报了会 recapture）· Cost Seg 是美国概念 · 去掉',
      '分配方式描述：HSA → 401(k) → Mega · 加拿大版变 RRSP → FHSA → TFSA',
      '搬家场景提示：去掉 Convenience Rule（美国专属）· 术语州 / 省自动切',
    ],
  },
  {
    theme: '常见误区模块',
    color: '#A990B5',
    items: [
      '模态框头部换成三徽章（分类色 · HOT 标 · 序号）· 更清晰',
      '内容排版改好了：加粗 / bullet / 编号列表 · 不再是一坨文字',
      '问题前面的 × 变成红色 Q 徽章 · 更像问答',
    ],
  },
  {
    theme: '幕后清理',
    color: '#6B6B6B',
    items: [
      '删了 975 行不用的代码（死组件 + 死常量）',
      '文件从 898KB 降到 725KB · 加载快 ~20%',
      '版本日志移到独立的 CHANGELOG.md',
    ],
  },
  {
    theme: '小彩蛋',
    color: '#0D0D0D',
    items: [
      '月到手详情底部有个「仿真 paystub ↗」· 点开看你的工资单长什么样（纯娱乐）',
    ],
  },
];
...
const ChangelogModal = ({ onClose }) => (
  ...
  >
    <div
      onClick={(e) => e.stopPropagation()}
      style={{
        background: C.bg, width: '100%', maxWidth: 440,
        maxHeight: '92vh', overflow: 'auto',
    ...
    >
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{
            ...
            ※ 给 XHS beta 看的用户版记录 · 每项都是你能感知的改动
            <br/>
            <span style={{ fontFamily: F_MONO, color: C.info, fontSize: 9, letterSpacing: '0.08em' }}>
              BUILD 2026-04-23-16 · Tailwind CSS 修复版
            </span>
          </div>
        </div>
        <button
          ...
          style={{
            background: 'transparent', border: 'none',
            fontSize: 22, color: C.mute, cursor: 'pointer',
            padding: 0, lineHeight: 1, width: 28, height: 28,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            flexShrink: 0,
          }}
        >×</button>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {CHANGELOG_USER_FACING.map((section, sIdx) => (
          <div key={sIdx} style={{
            padding: '10px 12px',
            background: C.card,
            borderRadius: 8,
            ...
            <div style={{
              ...
              color: C.ink, marginBottom: 6, letterSpacing: '-0.005em',
            }}>
              {section.theme}
            </div>
            {section.items.map((item, iIdx) => (
              <div key={iIdx} style={{
                ...
                <span style={{
                  color: section.color, fontWeight: 700, flexShrink: 0,
      ...
      <div style={{
        ...
        § 感谢帮我打磨这个工具 · 有 bug 或建议直接 DM
        <br/>
        <span style={{ color: C.muteLite, fontSize: 9 }}>
          @JMJ · XHS · tax.jmjvc.us
        </span>
      </div>
    </div>
  </div>
...
const Header = ({ preset, setPreset, isModified, saveStatus, onEdit, onPickPersona, onShowWorksheet, country, onChangeCountry }) => (
  <div className="flex items-center justify-between px-3 py-2"
    ...
    {/* 左侧 · Logo（v142: 内测徽章挪到首页 landing · 这里不再显示） */}
...
const Wizard = ({ initial, onComplete, onCancel, country }) => {
  ...
  if (step === 2) {
    ...
    return (
      ...
      >
        ...
        {/* v117: MFJ 双工作州 · 配偶工作州单独设置（仅 US · CA 无跨州概念）*/}
        {!isCA && draft.filingStatus === 'MFJ' && (draft.spouseW2 || 0) > 0 && (
          <div style={{
            ...
            <div style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              marginBottom: draft.spouseWorkState ? 8 : 0,
            }}>
              <div>
                <div style={{ fontSize: 12, color: C.ink, fontFamily: F_BODY, fontWeight: 700 }}>
                  配偶工作州不同
                </div>
                <div style={{ fontSize: 10, color: C.mute, fontFamily: F_BODY, marginTop: 2 }}>
                  例：你 NY · 配偶 NJ / 你 NJ · 配偶 NY
                </div>
              </div>
              <button
                onClick={() => update({
                  spouseWorkState: draft.spouseWorkState
                    ? ''
                    : (draft.state === 'NJ' ? 'NY' : draft.state === 'NY' ? 'NJ' : ''),
                  spouseWorkStateDays: draft.spouseWorkState ? undefined : (draft.spouseWorkStateDays ?? 100),
                })}
                style={{
                  padding: '5px 12px', borderRadius: 6,
                  background: draft.spouseWorkState ? C.ink : C.card,
                  border: `1px solid ${draft.spouseWorkState ? C.ink : C.line}`,
                  color: draft.spouseWorkState ? '#FFF' : C.ink2,
                  fontSize: 10, fontFamily: F_BODY, fontWeight: 600, cursor: 'pointer',
                }}
              >
                {draft.spouseWorkState ? '已开' : '设置'}
              </button>
            </div>
            {draft.spouseWorkState && (
              <>
                <div style={{
                  fontSize: 11, color: C.ink2, fontFamily: F_BODY,
                  fontWeight: 600, marginBottom: 6, marginTop: 2,
                }}>
                  配偶工作州
                </div>
                <StateSelect
                  value={draft.spouseWorkState}
                  onChange={(v) => update({ spouseWorkState: v })}
                  country={country}
                />
                {(RECIPROCAL_AGREEMENTS[draft.state] || []).includes(draft.spouseWorkState) && (
                  <div style={{
                    fontSize: 10, color: C.save, fontFamily: F_BODY, lineHeight: 1.5,
                    background: C.saveBg, padding: '6px 10px', borderRadius: 6, marginTop: 8,
                  }}>
                    ✓ Reciprocal 互惠 · 配偶工作州不扣税
                  </div>
                )}
                {CONVENIENCE_RULE_STATES[draft.spouseWorkState] && (draft.spouseWorkStateDays ?? 100) < 100 && (
                  <div style={{
                    fontSize: 10, color: C.warn, fontFamily: F_BODY, lineHeight: 1.5,
                    background: C.warnBg, padding: '6px 10px', borderRadius: 6, marginTop: 8,
                  }}>
                    † {STATE_BRACKETS[draft.spouseWorkState].label} 有 Convenience Rule
                  </div>
                )}
              </>
            )}
                ...
                return (
                  <div key={p.id || idx} style={{
                    ...
                    <div style={{ marginBottom: 8 }}>
                      ...
                      }}>{isCA ? '房产所在省' : '房产所在州'}</div>
                      <StateSelect
                        value={(() => {
                          // v133: 国家切换后 · 如果 property 还存着跨国的 state · 显示居住省/州作兜底
                          if (country === 'CA' && p.state && !CA_PROV_BRACKETS[p.state]) return draft.state;
                          if (country !== 'CA' && p.state && CA_PROV_BRACKETS[p.state]) return draft.state;
                          return p.state || draft.state;
                        })()}
                        onChange={(v) => {
                          const next = [...props];
                    ...
                    <Slider label="房贷年利息" value={Number(p.mortInt) || 0}
                      ...
                      min={0} max={60000} step={500}
                    />
                    <Slider label={isCA ? '地税 (市政税)' : '地税 + 学校税'} value={Number(p.propertyTax) || 0}
                      onChange={(v) => {
                        const next = [...props];
                      ...
                      min={0} max={50000} step={500}
                    />
                    {/* v130: rental type 追加收入/运营/折旧 + Cost Seg 勾选
                            没这些数字 rentalNet 默认为负 · AGI 失真
                            Cost Seg 默认关 · 勾后才计入省税
                        v135: CA 模式下折旧 + Cost Seg 不适用 · 改为 CCA 说明卡
                              （加拿大 rental 用 CCA Class 1 4% DB · 可选 · 触发 recapture） */}
                    {p.type === 'rental' && (
                      <div style={{
                        borderTop: `1px dashed ${C.lineLite}`,
                        paddingTop: 8, marginTop: 6,
                      }}>
                        <Slider label="年租金收入" value={Number(p.rentalIncome) || 0}
                          onChange={(v) => {
                            const next = [...props];
                            next[idx] = { ...next[idx], rentalIncome: v };
                            update({ properties: next });
                          }}
                          min={0} max={300000} step={1000}
                        />
                        <Slider label="运营费用" value={Number(p.rentalExpenses) || 0}
                          onChange={(v) => {
                            const next = [...props];
                            next[idx] = { ...next[idx], rentalExpenses: v };
                            update({ properties: next });
                          }}
                          min={0} max={100000} step={500}
                          hint={isCA
                            ? '保险 · 修理 · 管理费 · 水电（空置期）· 广告（不含房贷/地税）'
                            : '保险 · HOA · 修理 · 管理费 · 广告（不含房贷/地税）'}
                        />
                        {!isCA ? (
                          <>
                            <Slider label="折旧 (27.5 年)" value={Number(p.depreciation) || 0}
                              onChange={(v) => {
                                const next = [...props];
                                next[idx] = { ...next[idx], depreciation: v };
                                update({ properties: next });
                              }}
                              min={0} max={60000} step={500}
                              hint="一般 = 建筑价值 / 27.5 · 估算：地税 × 53"
                            />
                            <div style={{ marginTop: 6 }}>
                              <Toggle
                                ...
                                onChange={(v) => {
                                  const next = [...props];
                                  next[idx] = { ...next[idx], costSeg: v };
                                  update({ properties: next });
                                }}
                                hint="加速折旧 · 研究费 ~$4.5K · 勾选后才计入省税"
                              />
                            </div>
                          </>
                        ) : (
                          <div style={{
                            ...
                            <div style={{ color: C.ink2, marginBottom: 3 }}>
                              CRA 用 <b>Class 1</b>（1988 后建筑）4% DB · 可选不报。
                            </div>
                            <div style={{ color: C.mute }}>
                              ※ 报 CCA 省当年税 · 但卖房时触发 <b style={{ color: C.ink2 }}>recapture</b> 全收 · 混合用途还可能失 PRE
                            </div>
                            <div style={{ color: C.muteLite, marginTop: 4, fontSize: 9 }}>
                              § 保守做法：纯出租报 CCA · 自住过就别报
                            </div>
                          </div>
                        )}
                      </div>
  ...
  return (
    <WizardShell
      ...
      isLast
    >
      {/* CA: RRSP · US: 401k · v115 MFJ dual earner → 2x
          v132: 换成 ContribEditSlider · 显示 delta 消除「已存 vs 再存」歧义 */}
      <ContribEditSlider
        label={isCA ? '今年已存 RRSP' : (draft.filingStatus === 'MFJ' && (draft.spouseW2 || 0) > 0 ? '今年已存 401(k) · 夫妻合计' : '今年已存 401(k)')}
        value={draft.k401}
        onChange={(v) => update({ k401: v })}
        baseline={draft._k401Baseline}
        min={0}
        max={isCA ? CA_RRSP_LIMIT : (draft.filingStatus === 'MFJ' && (draft.spouseW2 || 0) > 0 ? K401_LIMIT_2025 * 2 : K401_LIMIT_2025)}
        step={500}
        hint={isCA
          ? `2025 上限 $${CA_RRSP_LIMIT.toLocaleString()} 或 18% × 上年收入`
          : (draft.filingStatus === 'MFJ' && (draft.spouseW2 || 0) > 0
              ? `夫妻各 $${K401_LIMIT_2025.toLocaleString()} · 合计 $${(K401_LIMIT_2025 * 2).toLocaleString()}`
              : `2025 上限 $${K401_LIMIT_2025.toLocaleString()}`)
        }
      />
      {/* v128: YTD 估算器 · 用户按工资 % 或每期 $ 反推已存金额 */}
      <ContribYTDCalc
        label={isCA ? 'RRSP' : '401(k)'}
        annualBase={(draft.w2 || 0) + (draft.filingStatus === 'MFJ' ? (draft.spouseW2 || 0) : 0)}
        maxCap={isCA ? CA_RRSP_LIMIT : (draft.filingStatus === 'MFJ' && (draft.spouseW2 || 0) > 0 ? K401_LIMIT_2025 * 2 : K401_LIMIT_2025)}
        onApply={(v) => {
          const rounded = Math.round(v / 500) * 500;
          // v132: 同时更新 _k401Baseline · YTD 应用后才是「已存」baseline · 之后再拖显示为「再存」
          update({ k401: rounded, _k401Baseline: rounded });
        }}
        defaultMode="pct"
        defaultPct={6}
      />
          ...
          {draft.hdhp && (
            <>
              <ContribEditSlider
                ...
                value={draft.hsa}
                onChange={(v) => update({ hsa: v })}
                baseline={draft._hsaBaseline}
                min={0}
                max={draft.filingStatus === 'MFJ' ? HSA_LIMIT_2025.Family : HSA_LIMIT_2025.Self}
                step={100}
              />
              {/* v128: HSA 一般是按每期 $ 而非 %（HSA % of salary 少见）· 默认 amt 模式 */}
              <ContribYTDCalc
                label="HSA"
                annualBase={(draft.w2 || 0) + (draft.filingStatus === 'MFJ' ? (draft.spouseW2 || 0) : 0)}
                maxCap={draft.filingStatus === 'MFJ' ? HSA_LIMIT_2025.Family : HSA_LIMIT_2025.Self}
                onApply={(v) => {
                  const rounded = Math.round(v / 100) * 100;
                  update({ hsa: rounded, _hsaBaseline: rounded });
                }}
                defaultMode="amt"
                defaultAmt={150}
              />
      ...
      {/* v131: §125 保险 premium · 折叠段 · 默认关 · 填了更真实
              三重减税（fed + state + FICA）· 和 401k 不一样（401k 只减 fed/state）
              另：显示居住州 PFML 自动估算 · 纯信息 · 不用用户填 */}
      {!isCA && (
        <PayrollBenefitsSection
          draft={draft}
          update={update}
      ...
      <div style={{
        ...
        {/* v131: 月到手 · 最直观 */}
        {previewCalc.monthlyTakeHome > 0 && (
          <div style={{
            marginTop: 8, paddingTop: 8,
            borderTop: `1px dashed ${C.heroMute}33`,
            fontSize: 11, color: C.heroMute,
          }}>
            每月到手现金 <b style={{ color: '#A8D5B8', fontFamily: F_NUM, fontSize: 14 }}>
              ${fmt(Math.round(previewCalc.monthlyTakeHome))}
            </b>
            &nbsp;·&nbsp;
            双周 <b style={{ color: C.heroInk, fontFamily: F_NUM }}>
              ${fmt(Math.round(previewCalc.biweeklyTakeHome))}
            </b>
          </div>
        )}
      </div>
...
const CountryPicker = ({ onPick }) => {
  // v142: 内测徽章 + changelog 模态框（从 Header 挪到这里）
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 430 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 4,
              color: '#FFF',
              fontSize: 9, fontFamily: F_BODY, fontWeight: 700,
        ...
        <div style={{
          ...
          Tax Year 2025 · 内测 v2.0.1 · Build 2026-04-23-16
        </div>
      </div>
      {/* v142: Changelog 模态框 · 点徽章打开 */}
...
export default function App() {
  ...
  // v119: 计算已有供款的 baseline · tuner 初始位置反映用户已执行的比例
  // 如 401k 已存 $29,500 / $47,000 cap = 63% · tuner 初始 pct 应为 63%
  // v125: 排除 itemizeDone · 已实现状态不是可操作项
  const tunerBaselinePct = useMemo(() => {
    const contribOpps = opps.filter(o =>
      o.type !== 'warning' && o.id !== 'itemizeDone' && (o.saving || 0) > 0 && (o.contrib || 0) > 0
    );
    if (contribOpps.length === 0) return 0;
    // 现有供款（按机会类型累加）
    // max401k 的 contrib 是 gap · gap = cap - k401_already → already = cap - gap → pct = already/cap
    // 简单估：已存 k401 + hsa + solo401k 等 · 相对于 "已存 + 还能存" 总额
    const alreadyContributed =
      (inputs.k401 || 0) +
      (inputs.hsa || 0) +
      (inputs.solo401k || 0) +
      (inputs.hsa2 || 0);
    const gapTotal = contribOpps.reduce((s, o) => s + (o.contrib || 0), 0);
    const totalCap = alreadyContributed + gapTotal;
    if (totalCap < 1) return 0;
    // v121: 精度到 0.1% · 让 slider step 0.1 能精确落在 baseline 位置上
    return Math.min(100, Math.round((alreadyContributed / totalCap) * 1000) / 10);
  }, [opps, inputs]);

  // v61: 调节器 pct 状态提升到 App 根 · Hero 表头跟随滑块动
  // v119: 初始位置 = baseline（反映已执行的部分）
  // v120: 真正把初始值设成 tunerBaselinePct · slider 代表「总进度」而非「额外增量」
  //       useState 初始化器只在首次 mount 跑一次 · 此时 tunerBaselinePct 已由上方 useMemo 算好
  const [tunerPct, setTunerPct] = useState(tunerBaselinePct);
  // v123: 分配模式 · 用户在 SavingsTuner 里切换 · 影响 Hero 的「少交的税」显示
  //       默认「priority · 实用派」（最接近真实行为）· 可切「proportional · 平均」/「efficient · 最优」
  const [tunerMode, setTunerMode] = useState('priority');

  // v61: 基于 pct 的预览 calc · Hero 显示这个版本（滑块 0% 时与原 calc 完全相同）
  // v125: 排除 itemizeDone · 否则会把已实现 saving 再加到 totalTaxSaved · Hero 显示数字翻倍
  const tunerPreview = useMemo(() => {
    const contribOpps = opps.filter(o =>
      o.type !== 'warning' && o.id !== 'itemizeDone' && (o.saving || 0) > 0 && (o.contrib || 0) > 0
    );
    const freeOpps = opps.filter(o =>
      o.type !== 'warning' && o.id !== 'itemizeDone' && (o.saving || 0) > 0 && !(o.contrib > 0)
    );
    const freeSaving = freeOpps.reduce((s, o) => s + (o.saving || 0), 0);

    // v120: scale 语义从「额外增量比例」改为「相对 baseline 的偏移比例」
    //   · pct === baselinePct → scale = 0（当前状态 · 不变）
    //   · pct === 100         → scale = 1（满档 · 填满所有 gap）
    //   · pct === 0           → scale = -baselinePct/(100-baselinePct)（倒推 · 撤回所有已存）
    //   · 线性插值 · 统一公式：scale = (pct - baselinePct) / (100 - baselinePct)
    //   · 老行为兼容：baselinePct === 0 时退化为 pct/100
    // v122: 正向 scale 按优先级顺序填 · 不再等比 · 聚合 $ 相同但分配不同 · 总税省更多（HSA 先满）
    // v123: 分配模式由用户选 · proportional / priority / efficient
    const denom = 100 - tunerBaselinePct;
    const scale = denom > 0 ? (tunerPct - tunerBaselinePct) / denom : 0;
    const allocation = allocateByMode(contribOpps, scale, tunerMode);
    const contribSaving = allocation.totalSaving;
    const contribAmt = allocation.totalContrib;
    // Free opps 不按优先级 · 仍用 proportional · 倒推归零（"换州住"/"慈善"没有"已做"概念）
    const freeScale = Math.max(0, scale);
    const freeSavingScaled = Math.round(freeSaving * freeScale);
    ...
    // v139: 边际税率 recomputation · tuner 填 contrib 后 fedTaxable 降了 · 可能跌档
    //   简化：newFedTaxable ≈ oldFedTaxable - contribAmt（假设所有 contrib 减 AGI · Roth 等少数例外忽略）
    //   marginalPayroll（FICA/CPP+EI 那部分）基于 totalW2 · tuner 不动 gross · 保持不变
    const newFedTaxable = Math.max(0, (calc?.fedTaxable || 0) - contribAmt);
    const oldStateTaxable = calc?.stateTaxable != null ? calc.stateTaxable : (calc?.fedTaxable || 0);
    const newStateTaxable = Math.max(0, oldStateTaxable - contribAmt);
    const oldMarginalFed = calc?.marginalFed || 0;
    const oldMarginalState = calc?.marginalState || 0;
    const oldMarginalCombined = calc?.marginalCombined || (oldMarginalFed + oldMarginalState);
    const oldMarginalPayroll = Math.max(0, oldMarginalCombined - oldMarginalFed - oldMarginalState);
    let newMarginalFed = oldMarginalFed;
    let newMarginalState = oldMarginalState;
    if (country === 'CA') {
      if (typeof CA_FED_BRACKETS !== 'undefined' && CA_FED_BRACKETS) {
        newMarginalFed = marginalRate(newFedTaxable, CA_FED_BRACKETS);
      }
      const provRules = CA_PROV_BRACKETS[inputs.state];
      if (provRules?.brackets) {
        newMarginalState = marginalRate(newStateTaxable, provRules.brackets);
      }
    } else {
      const fedBrackets = FED_BRACKETS[inputs.filingStatus];
      if (fedBrackets) {
        newMarginalFed = marginalRate(newFedTaxable, fedBrackets);
      }
      const stateRules = STATE_BRACKETS[inputs.state];
      const stateBrackets = stateRules?.[inputs.filingStatus] || stateRules?.Single;
      if (stateBrackets) {
        newMarginalState = marginalRate(newStateTaxable, stateBrackets);
      }
    }
    ...
    return {
      ...
      // v138: 月/双周到手也要跟着 tuner 更新 · 否则 Hero 上方「≈ $X/月」和 4 格频率不变
      monthlyTakeHome: (baseCash + totalTaxSaved - contribAmt) / 12,
      biweeklyTakeHome: (baseCash + totalTaxSaved - contribAmt) / 26,
      // v139: 边际 recomputation · 让分源月到手精算也跟 tuner 联动
      marginalFed: newMarginalFed,
      marginalState: newMarginalState,
      marginalCombined: newMarginalFed + newMarginalState + oldMarginalPayroll,
      // fedTaxable 也应该更新 · 让 LTCG bracket lookup 准确
      fedTaxable: newFedTaxable,
      effectiveRate: baseGross > 0 ? (baseTotalTax - totalTaxSaved) / baseGross : 0,
      // 标记：预览模式 + 滑块位置
      _tunerPct: tunerPct,
      _tunerBaselinePct: tunerBaselinePct,
      _tunerDelta: tunerPct - tunerBaselinePct, // 正 = 多做 · 负 = 倒推 · 0 = 当前状态
      _totalTaxSaved: totalTaxSaved,
      _contribAmt: contribAmt,
      _baseTotalTax: baseTotalTax,
      _baseCash: baseCash,
    };
  }, [opps, calc, tunerPct, tunerBaselinePct, tunerMode]);

  // v61: 切换 persona / 编辑时重置滑块（滑块代表整体进度位置）
  // v120: reset 目标从 0 改为 tunerBaselinePct · 反映新 inputs 下的已存状态
  React.useEffect(() => {
    setTunerPct(tunerBaselinePct);
  }, [inputs, tunerBaselinePct]);
      ...
      <div style={{ background: C.bg, minHeight: '100vh', fontFamily: F_BODY }}>
        <div className="mx-auto" style={{ maxWidth: 440, paddingBottom: 40, backgroundColor: theme.bodyBg, minHeight: '100vh' }}>
          ...
          <div className="px-3 pt-3">
            ...
            <SavingsTuner opps={opps} calc={calc} inputs={inputs} pct={tunerPct} setPct={setTunerPct} baselinePct={tunerBaselinePct} mode={tunerMode} setMode={setTunerMode} />
